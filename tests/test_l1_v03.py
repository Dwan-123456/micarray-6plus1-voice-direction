...
def test_imcra_emits_exact_20ms_hops_for_arbitrary_input_chunking():
    ...
    assert ready.frequencies_hz.size == 338
    assert ready.frequencies_hz[0] >= 80.0 and ready.frequencies_hz[-1] <= 8_000.0
    assert ready.noise_psd.shape == ready.smoothed_psd.shape == ready.minimum_psd.shape == ready.spp.shape == (7, 338)
    assert ready.conditional_smoothed_psd.shape == ready.conditional_minimum_psd.shape == (7, 338)
    assert ready.speech_absence_probability.shape == ready.posterior_snr.shape == ready.prior_snr.shape == (7, 338)
...
def test_cohen_2003_table_parameters_and_two_pass_state_are_exposed():
    ...
    assert config.algorithm_version == "cohen_imcra_2003_l1_v1"
    assert (
