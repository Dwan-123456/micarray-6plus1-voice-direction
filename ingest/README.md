...
sequence gap、timestamp/rate异常、设备重启、输入overflow和handoff丢块在这里转换为epoch重置。一个健康事件及其对应sequence gap只生成一次重置；采集边界会把连续handoff溢出合并为一个缺口事件。新epoch的绝对sample从0重新开始，窗口ID仍由`WindowAssembler`保持单调递增。
