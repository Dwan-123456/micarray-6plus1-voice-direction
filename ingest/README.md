# IngestCoordinator：v1.4.4唯一时间轴契约
...
当前v1.4.4没有`RecordingStore`或录音消费者。`ApplicationRuntime`把block交给L1 IMCRA、可选预降噪、`WindowAssembler`和1秒内存环。IMCRA hop在Coordinator分配sample区间后生成，并必须与block、epoch和sample边界严格一致。
