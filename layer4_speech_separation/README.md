...
匹配器先按`l3_bf_1_4khz_complex_coherence_v3`对完整音频执行512点Hann STFT、160点hop和1～4 kHz逐帧复频谱相干度，再按原Hub参考频带能量加权。该统计保留说话人的相位/时序身份，并容忍模型产生的全局极性翻转；末尾不足一帧补零，长录音分批计算以限制内存。音轨短于2秒、最高相干度低于0.50或候选分差小于0.025时，输出回退为原L3参考，避免把短轨或身份不明确的模型伪影送入试听和L5。

封存批处理随后执行`l3_bf_1_4khz_cross_track_penalty_v4`：依据每条L3长轨的`start_sample/end_sample`决定时间轴，把初选轨的两条匿名候选分别与其他ID的L3参考只在真实重叠区间内对齐比较。各轨时长和起点可以不同；跨epoch、无重叠或重叠不足2秒的组合不参与。如果1～4 kHz初选候选对某条其他ID参考的分数比同次“一拆二”的另一个候选至少高0.025，说明初选更像其他方向轨，最终改选其同胞候选。初选、最终索引、对方ID/角度、决定时间重叠范围、双候选交叉分数和切换差值均进入结果元数据。
...
sealed TrackAudioStreamHub long track (48 kHz, ID + angle + L2 count history)
  ...
  -> 1--4 kHz initial matcher -> decision-time aligned cross-track penalty
  -> one selected source preserving ID + angle
  -> Layer 5 CNN
```
