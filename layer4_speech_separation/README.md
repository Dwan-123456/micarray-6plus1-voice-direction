...
Test UI“合并”默认开启。开启时，匹配器按`l3_bf_1_4khz_complex_coherence_v3`对完整音频执行512点Hann STFT、160点hop和1～4 kHz逐帧复频谱相干度，再按原Hub参考频带能量加权，只保留高分候选。该统计保留说话人的相位/时序身份，并容忍模型产生的全局极性翻转；末尾不足一帧补零，长录音分批计算以限制内存。音轨短于2秒、最高相干度低于0.50或候选分差小于0.025时，输出回退为原L3参考，避免把短轨或身份不明确的模型伪影送入试听和L5。匹配分数、差值、模型revision及回退原因均进入结果元数据和离线job manifest。关闭“合并”时，L4仍计算相同匹配度，但只将其用于排序：两条模型原生16 kHz候选均写入临时试听缓存，高分标为A、低分标为B，分数相同时按模型原始顺序稳定排序；界面显示原ID、A/B及三位小数分数，不显示角度。两条候选分别进入同一个L5并把逐20 ms结果标在对应波形上。

每条最终L4 16 kHz波形完成峰值保护后立即运行项目现有DNSMOS P.835模型。原始SIG、BAK、OVRL（1～5）写入L4元数据，并按`(0.25×SIG + 0.25×BAK + 0.50×OVRL - 1) / 4`生成0～1综合MOS；所有L4预览标签末尾显示三位小数MOS。Runtime缓存这一CPU评分器供后续L4提交复用；L6直接消费已保存的MOS，不重复运行DNSMOS。
...
sealed TrackAudioStreamHub long track (48 kHz, ID + angle + L2 count history)
  ...
  -> merge ON: 1--4 kHz matcher -> one selected source preserving ID + angle -> Layer 5 CNN
  -> merge OFF: display both anonymous 16 kHz candidates as ID-A / ID-B -> Layer 5 each
```
