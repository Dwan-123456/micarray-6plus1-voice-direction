...
每条最终L4 16 kHz波形完成峰值保护后立即运行项目现有DNSMOS P.835模型。原始SIG、BAK、OVRL（1～5）写入L4元数据，并按`(0.25×SIG + 0.25×BAK + 0.50×OVRL - 1) / 4`生成0～1综合MOS；所有L4预览标签末尾显示三位小数MOS。Runtime缓存这一CPU评分器并在后续L6复用，不重复加载第二份模型。
