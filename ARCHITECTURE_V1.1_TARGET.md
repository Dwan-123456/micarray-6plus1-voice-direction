# 6+1 麦克风阵列项目 1.2.1：MUSIC 与公开方向 ID 架构

状态：**1.2.1代码已整合并进入发布验收；真实阵列、诊室声场与长时间运行验收仍按本文门禁继续执行，不以自动测试替代。**

发布版本：项目`1.2.1`，对应不可变标签`v1.2.1`。不得移动、覆盖或重写已经发布的`v1.0.1`、`v1.1.1`、`v1.1.2`及其他历史标签。
...
- 首版最多输出 3 个候选，圆周 NMS 最小间隔为 50°。
- 峰值选择必须原生处理数组首尾相邻，`359°` 和 `0°` 属于相邻角度。
- 无足够有效频点、协方差退化或模型阶数不可信时，返回可诊断的 blocked/degraded/failed 状态，不得静默复用上一窗伪谱冒充新观测。
- 原 SRP-PHAT、iterative multiple peak 与相关回退不再进入正式1.2.1主链；删除配置、运行时setter、UI开关和专属测试。若保留历史实现用于回归，只能放在明确的非运行时归档边界，不能被新pipeline导入。
...
- 1.2.1提供封存session的公共只读查询与Log UI；未由公开接口提供的数据仍必须明确显示不可用，不能绕过边界伪造。
...
9. **整合验收**：全量自动测试、性能记录、实机边界、CHANGELOG、语义版本与`v1.2.1`发布。
