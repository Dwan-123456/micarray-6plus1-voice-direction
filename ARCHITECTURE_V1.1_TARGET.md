...
- L2 必须获得连续的48 kHz、7个物理麦校准音频，并可访问DecisionWindow内最多160 ms历史；MUSIC实际使用滚动状态增量更新。HardwareMix仍不得进入协方差、导向矢量或MUSIC伪谱。
...
- `DecisionWindow [7680,8]`按20 ms节拍发布；160 ms同时是L3/L4上下文和L2可用历史上限，不代表L2每次都重新计算整段音频。
- L2维护按session/epoch/sample连续的滚动STFT与协方差状态。每个新DecisionWindow原则上只加入最近20 ms产生的新帧并移出超出160 ms历史的旧帧；禁止每20 ms从头重算160 ms STFT和全部协方差。
- `music.context_ms`固定不得超过160 ms；若未来需要更长MUSIC历史，必须先版本化修改公共时间契约，不能在L2内部静默读取不同时间范围。
...
- 输入：48 kHz、7个物理麦的连续滚动音频；可用历史上限固定为160 ms。
...
- MUSIC的160 ms历史与20 ms输出节拍不是同一概念：首轮只在滚动状态预热期间等待足够快照；预热完成后每20 ms继续发布新结果，不额外引入160 ms批处理等待。
...
- 在目标设备上基准160 ms滚动历史，验证50 Hz持续流水、预热后每20 ms有新MUSIC结果、队列不持续积压；L2初始预算为p95不超过15 ms、硬门限小于20 ms。记录STFT、协方差、eigh、伪谱、MDL和关联的分项耗时。
