...
状态：**项目`1.3.3`已正式发布，发布基线为不可变标签`v1.3.3`。真实阵列、诊室声场与长时间运行验收仍按本文门禁继续执行，不以自动测试替代。**

发布版本：项目`1.3.3`，最终基线`v1.3.3`固定指向本次发布提交。不得移动、覆盖或重写已经发布的`v1.0.1`、`v1.1.1`、`v1.1.2`、`v1.2.1`、`v1.2.2`、`v1.2.3`、`v1.2.4`、`v1.3.1`、`v1.3.2`、`v1.3.3`及其他历史标签。
...
- IMCRA对7个物理麦分别统计并发布0～10000 Hz噪声PSD/SPP，Wiener预降噪同样作用于0～10000 Hz；L2 Gate概率证据带使用100～1500 Hz，HardwareMix不参与统计或降噪。
...
- 两条既有轨迹后来进入50°以内时，仅当滚动200 ms内两轨都有观测、观测呈至少两次交替且不存在同窗双峰时，才判定为同源重复轨。归并保留confirmed优先、随后更早、存在概率更高的权威ID，吸收另一轨较新的IMM状态与观测证据；同窗独立双峰不得合并。
