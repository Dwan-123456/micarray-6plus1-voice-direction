...
L4：Test UI手动提交，统一48→16 kHz并完成一/二人路由、分离和匹配
    ...
    → 1～4 kHz复频谱相干初筛或低可信回退L3参考
    → 按决定时间对齐其他L3方向轨，惩罚更像其他ID的初选候选
...
- 每条L3输入的两候选最多发布一个。原L3 BF参考经相同重采样后，以512点Hann STFT、160点hop在1～4 kHz计算逐帧复频谱相干度并按参考频带能量加权；复内积保留相位和时序身份，同时以绝对值容忍全局极性翻转。音轨短于2秒、最高相干度低于0.50或两分数差小于0.025时，不发布不可靠模型候选，改用同一条L3参考音频并记录回退原因；其他平分仍固定候选索引0。
- 可靠初选进入封存批次交叉轨惩罚。以父L3轨的`start_sample/end_sample`为权威决定时间，将其两候选分别与每条同epoch其他ID的L3参考裁到绝对时间交集后计算同一1～4 kHz分数；不同起点和不同时长不得按数组首端强行对齐。无交集或交集不足2秒不参与。若初选候选对任一其他ID参考的分数比同胞候选至少高0.025，则最终切换到同胞候选并继承原ID/角度；完整比较、对方ID/角度、重叠决定时间和切换原因必须审计。
