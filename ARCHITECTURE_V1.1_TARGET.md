...
状态：**项目`1.3.5`已正式发布，发布基线为不可变标签`v1.3.5`。真实阵列、诊室声场与长时间运行验收仍按本文门禁继续执行，不以自动测试替代。**

发布版本：项目`1.3.5`，最终基线`v1.3.5`固定指向本次发布提交。不得移动、覆盖或重写已经发布的`v1.0.1`、`v1.1.1`、`v1.1.2`、`v1.2.1`、`v1.2.2`、`v1.2.3`、`v1.2.4`、`v1.3.1`、`v1.3.2`、`v1.3.3`、`v1.3.5`及其他历史标签。
...
面向首次接触项目的可视化总图、逐层输入/输出/内部处理单元和操作流程见[`docs/COMPLETE_ARCHITECTURE_AND_USAGE.md`](docs/COMPLETE_ARCHITECTURE_AND_USAGE.md)。本文作为1.3.3正式契约，使用手册不建立第二份配置来源。
...
L4：L3长轨封存后由Test UI自动提交，统一48→16 kHz并完成一/二人路由、分离和匹配
    ...
    → 合并关闭：保留两条候选并按匹配度降序标记A/B，界面显示原ID、A/B及分数（不显示角度）
    → 最终L4波形运行DNSMOS，记录SIG/BAK/OVRL并显示0～1综合MOS
    ↓ 整批L4完成后自动且仅一次运行
L5：NVIDIA Frame-VAD输出逐20 ms概率序列
    → 与每个320样本hop严格对齐；整轨摘要取完整序列连续3帧最大均值
    → 结果按精确ID回写L4预览条（概率 + Voice/Non-Voice）
    ↓ 用户手动点击，且L4采用“保留双候选”模式
L6：A整轨必提CAMPPlus声纹；B满足差≤0.20、匹配>0.50、MOS>0.30才提取
    → 入选整轨声纹两两相似度 + 平均链接AHC聚为0～3人
    → 一个声纹关联多条A/B音轨，按录音绝对时间线逐20 ms填入
    → 重叠音频保留MOS较高者，无音频处补静音
    → 删除首尾静音；内部超过2秒的静音压缩到2秒
    → 按声纹显示16 kHz Speaker A/B/C音频
...
- Gate只使用与窗口末端对齐的当前20 ms IMCRA概率，达到阈值才运行MUSIC；前一个20 ms概率不再参与平均或阻断当前判决。L2保留按精确ID接收在线语义反馈后强制放行的兼容接口，但正式L5审计和渐进L5 preview都不把语义回传L2，因此1.3.5普通运行不会触发语义强制放行。ID继续按2秒绝对sample TTL推进到coasting/超时；所有仍在有效TTL内的正式coasting ID都可在数量与角距限制内作为公共方向送入L3。
...
- `IMCRA噪声白化`默认开启。L1在每个20 ms hop以低阵列SPP控制复数外积递归更新，持续发布完整7×7空间噪声协方差；L2直接读取DecisionWindow中READY协方差，经频率插值、收缩和loading后同时白化观测协方差与steering，不再估计私有噪声矩阵。缺少READY快照、完整协方差或Cholesky失败时标记`unavailable`并退回未白化路径。
...
- 原 SRP-PHAT、iterative multiple peak 与相关回退不再进入正式1.3.3主链；删除配置、运行时setter、UI开关和专属测试。若保留历史实现用于回归，只能放在明确的非运行时归档边界，不能被新pipeline导入。
...
- 首次高new概率且不与现存轨迹重复的观测立即分配新ID；滚动500 ms内累计至少10次关联观测且存在概率达标后确认。ID一经分配，在同一session内不得复用。
- 未匹配观测恢复旧ID时，同时检查该轨迹的IMM预测角和最后一次真实观测角；任一圆周距离不超过50°即优先关联旧ID并禁止重复birth，避免短时预测漂移把同一声源拆成新ID。
- 两条既有轨迹后来进入50°以内时，仅当滚动500 ms内两轨都有观测、观测呈至少两次交替且不存在同窗双峰时，才判定为同源重复轨。归并保留confirmed优先、随后更早、存在概率更高的权威ID，吸收另一轨较新的IMM状态与观测证据；同窗独立双峰不得合并。
...
- 1.3.3提供封存session的公共只读查询与Log UI；未由公开接口提供的数据仍必须明确显示不可用，不能绕过边界伪造。
...
9. **整合验收**：全量自动测试、性能记录、实机边界、CHANGELOG、语义版本与`v1.3.5`发布。
