# 6+1 麦克风阵列项目 1.2.2：MUSIC 与公开方向 ID 架构

状态：**1.2.2代码已整合并进入发布验收；真实阵列、诊室声场与长时间运行验收仍按本文门禁继续执行，不以自动测试替代。**

发布版本：项目`1.2.2`，对应不可变标签`v1.2.2`。不得移动、覆盖或重写已经发布的`v1.0.1`、`v1.1.1`、`v1.1.2`、`v1.2.1`及其他历史标签。
...
- 原 SRP-PHAT、iterative multiple peak 与相关回退不再进入正式1.2.2主链；删除配置、运行时setter、UI开关和专属测试。若保留历史实现用于回归，只能放在明确的非运行时归档边界，不能被新pipeline导入。
...
- 1.2.2提供封存session的公共只读查询与Log UI；未由公开接口提供的数据仍必须明确显示不可用，不能绕过边界伪造。
...
9. **整合验收**：全量自动测试、性能记录、实机边界、CHANGELOG、语义版本与`v1.2.2`发布。
