...
- Kalman关闭时，confirmed ID使用最近3秒的圆周观测历史判断短时静止：至少70%观测位于圆周均值±10°时，输出改为持续更新的圆周均值。静止期间，滚动1秒内第1～3个超出均值±20°的观测不得移动输出和关联锚点；第4个外点立即解除静止并跟随当前观测。漏检时保持静止均值或最后可信观测，不做角速度外推；所有角差正确处理359°/0°。
