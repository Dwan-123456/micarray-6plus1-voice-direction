...
本目录已纳入项目1.2.4。运行录音详情使用公共`(session_id, stream_epoch, track_id)`，显示轨迹时间线摘要并提供逐ID连续补偿音频试听；该音频与Test UI及L4 CNN输入一致，重叠L3原始窗不再重复保存。旧v3和L1-only录音明确不含算法ID。
