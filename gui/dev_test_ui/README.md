# Development Test UI：项目1.3.4开发线
...
- 下右L6：已删除“运行L6”按钮。每批L4与L5成功完成后立即自动加载CPU CAMPPlus并运行L6；同一测试用例重复发送L4时，L6也按新批次重跑并替换旧结果。每条A提取声纹；B仅在A/B匹配差不超过0.20、B匹配度大于0.50且MOS大于0.30时参与；单人旁路作为该来源唯一A轨参与。界面按声纹显示Speaker A/B/C、关联音轨数、来源L2 ID、平均MOS、压缩后时长和波形。
