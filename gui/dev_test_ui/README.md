...
本UI只消费同一个ApplicationRuntime快照，不得重开设备、重建时间轴或在界面线程运行算法。普通启动使用真实麦克风并保留实时CDC/DOA极坐标显示；数据管理系统发起模拟测试时，`RecordingReplaySource`只读取已登记的原始8ch音频，不读取录音时的CDC热力图；后台重新运行Gate、MUSIC、ID和L3，并把本次L2计算得到的DOA快照提交给极坐标控件。实时链到Hub封存为止；L3完全排空并封存后自动按L4栏预选模型提交，L4整批完成后由同一后台工作自动且仅一次运行L5。
...
- 下中L4/L5：两层共用一个音频面板，仅在面板标题显示`L4 / L5`。已删除“合并”开关，Test UI每次固定保留两条16 kHz候选，按1～4 kHz匹配度降序标记A/B（A高、B低），以原ID、A/B和三位小数分数显示供试听；单人轨保留唯一旁路音频。最终L4波形运行DNSMOS，完整标签语义为`1A · 匹配度 0.941 · MOS 0.823`；界面紧凑显示为第一行`1A·匹配度0.941`、第二行`MOS 0.823`。所有L4输出自动进入L5，逐20 ms人声结果作为本栏波形黄色区间呈现。试听缓存是L4原生16 kHz单声道PCM16 WAV；播放端解析RIFF/WAV后以16 kHz直接建立声卡流，不做16→48 kHz重采样。
- 下中L4/L5：逐轨保存L4输出WAV并提供与L3一致的ID、角度、时长、波形和播放控件；不提供手动“发送到L5”按钮。L5自动一次读取完整长音频并为每个20 ms hop返回独立概率。
- 下右L6：已删除“运行L6”按钮。每批L4与L5成功完成后立即自动加载CPU CAMPPlus并运行L6；同一测试用例的新一轮L3封存触发L4后，L6也按新批次重跑并替换旧结果。每条A提取声纹；B仅在A/B匹配差不超过0.20、B匹配度大于0.50且MOS大于0.30时参与；单人旁路作为该来源唯一A轨参与。界面按声纹显示Speaker A/B/C、关联音轨数、来源L2 ID、平均MOS、压缩后时长和波形。
