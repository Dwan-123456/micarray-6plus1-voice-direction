...
class RotaryEmbedding(nn.Module):
    """Dependency-free rotary embedding used by the upstream MossFormer2 block."""
    ...
    def rotate_queries_or_keys(self, value):
        positions = torch.arange(value.shape[-2], device=value.device, dtype=self.freqs.dtype)
        phase = torch.outer(positions, self.freqs).repeat_interleave(2, dim=-1)
        rotary_dim = phase.shape[-1]
        rotated, remainder = value[..., :rotary_dim], value[..., rotary_dim:]
        phase = phase.to(dtype=value.dtype)
        rotated = rotated * phase.cos() + self._rotate_half(rotated) * phase.sin()
...
class FLASH_ShareA_FFConvM(nn.Module):
    ...
    def cal_attention(self, x, quad_q, lin_q, quad_k, lin_k, v, u, mask=None):
        ...
        quad_q, quad_k, lin_q, lin_k, v, u = map(
            lambda t: t.reshape(t.shape[0], -1, self.group_size, t.shape[-1]),
            (quad_q, quad_k, lin_q, lin_k, v, u),
        )

        if exists(mask):
            mask = mask.reshape(mask.shape[0], -1, 1, g)
        ...
        return map(
            lambda t: t.reshape(t.shape[0], -1, t.shape[-1])[:, :n],
