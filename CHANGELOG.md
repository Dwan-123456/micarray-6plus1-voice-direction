...
- **验证与边界**：新增停止按钮无参数调用回归测试；项目内一键安装成功路径、启动预检和离屏窗口生命周期通过，Ruff通过，全量pytest为`161 passed`。推送后继续以GitHub全新克隆验证可控失败和正常安装路径。L1/L2算法、校准参数、运行时数据契约、模型、Git LFS资产和发布标签均无变化。
