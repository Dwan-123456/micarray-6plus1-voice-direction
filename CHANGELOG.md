...
5. 本文件记录“发生了什么”；当前开发版本为`1.4.1`，最近正式架构以`ARCHITECTURE_V1.1_TARGET.md`为权威契约，发布基线为不可变标签`v1.3.6`，实际参数以`config/config.yaml`和代码为准。

## 2026-08-27 — 新增v1.3.6到1.4简明架构与使用说明

- **文档**：新增`docs/V1.3.6_TO_V1.4_OVERVIEW_AND_USAGE.md`并从根`README.md`提供入口，用面向首次接触者的简图说明二维远场角度模型、v1.3.6完整L1～L6链路、1.4轻量L1/L2开发目标和两种Development Test UI操作流程。
- **事实边界**：明确区分最多3个定位方向与最多2人同时分离；按实际代码记录v1.3.6的250～3400 Hz三频带概率、默认Gate 0.60、L4单人旁路和双人MossFormer2路径；把300～1500 Hz、Gate 0.80及3小时运行/每分钟1.7 MB内存增长标为1.4目标或待归档人工观察，不冒充已发布实现或正式验收。
- **未改变与验证**：L1～L6算法、Runtime、配置、接口、各UI、录音/数据管理、测试和模型资产均无变化；执行Markdown链接、关键事实和最终Git差异静态检查。无Git LFS对象新增或修改。
