...
- Runtime调度、性能快照字段、L1/L2/L3/L4算法、录音与数据管理、Production UI、模型和二进制资产均无变化，Git LFS资产无变化。
- **验证**：更新底栏初始布局文本测试；Development Test UI测试25项通过。
...
- L1采集、L2 MUSIC/ID、L3波束形成与试听、L4分类、正式录音/数据管理、Production UI、模型和二进制资产均无变化，Git LFS资产无变化。
...
- **类型**：可视化布局调整。
- 从L2 DOA/MUSIC右侧控件区移除`track_id/观测角/输出角/score/状态/新建/观测/L4概率`表格及其UI更新路径，控件区剩余空间改用弹性占位，避免留下空表格边框。
- MUSIC伪谱、圆形方向/ID颜色、Gate、Kalman、L2权威ID数据契约、L3试听拼接和L4概率算法均无变化；Git LFS资产无变化。

---

## 2026-08-19 — 修复模拟输入结束后L3试听缓存被误删

- **版本/标签**：Development Test UI试听缓存生命周期修复；未创建或移动版本标签。
- **类型**：L3模拟输入试听可用性与音轨质量过滤修复。
- 保留为维持绝对时间轴而插入的coasting/补洞等时静音，但不再将这些人工占位静音计入音轨质量过滤依据，避免模拟输入结束后误删已含有效L3声音的缓存。
- 队列排空期间真实L3结果带来的末尾连续静音不再单独删除整条已有效音轨；封存时统一按全轨声音占比判断，占比不超过30%的纯静音/低有效内容音轨仍删除。
- Center Mic、20 ms稳定hop、补洞长度、交叉淡化、L2权威ID键和关窗删除行为不变。
- 新增“有效声音后长时间coasting”与“队列排空时出现长静音尾”回归测试，验证时间轴仍等长且封存后可生成播放快照；Git LFS资产无变化。

---

## 2026-08-19 — 修复Development Test UI停止采集无法完成

- **版本/标签**：Development Test UI与Runtime停止生命周期修复；未创建或移动版本标签。
- **类型**：并发停止、队列清理与界面终态一致性。
- 大队列积压超过首次优雅排空期限时，强制取消阶段不再只固定等待1秒，而是在取消全部已登记窗口后为`commit`等处理线程提供一个完整的配置停止期限；仅当二次期限后仍存活才记录停止错误。
- 所有处理线程退出后清空L2/L3/L4及completion邮箱中的残留任务/EOS，释放滞留的320 ms窗口并让停止状态的队列深度归零；RecordingStore仍只在线程完全退出后关闭。
- Test UI在`runtime.stop()`返回后再次检查`runtime.active`：线程或录音会话仍存活时明确显示“停止采集失败”并保留停止按钮，不再误报“已完成”；成功停止后丢弃邮箱残留帧，不再把L2重新绘成LIVE。
- L1、L2/MUSIC与ID、L3、L4算法及正式录音格式均无变化；新增强制commit退出、残留帧抑制和UI停止终态测试，Git LFS资产无变化。

---

## 2026-08-19 — L3试听轨显示L2权威ID序号与稳定颜色

- **版本/标签**：Development Test UI显示调整；未创建或移动版本标签。
- **类型**：L2/L3身份可视化统一。
- 左下方向试听轨不再显示“最终角度”，改为直接显示L2权威`track_id`的十进制序号（如`1`、`2`、`10`、`20`），不添加前缀或补零。
- ID文字复用右上DOA/MUSIC圆图和方向表的同一稳定颜色函数，保证相同权威ID在两个区域始终同色；Center Mic对照显示不变。
- L1、L2 ID分配/MUSIC/Kalman、L3合成与缓存、L4、录音均无变化；更新L3试听行显示测试，Git LFS资产无变化。

---

## 2026-08-19 — L1预降噪显示历史平均频率增益

- **版本/标签**：Development Test UI显示调整；未创建或移动版本标签。
- **类型**：L1预降噪统计口径修正。
- 左上预降噪状态由最近一个音频块的瞬时平均频率增益改为当前`(session_id, stream_epoch)`内的历史平均频率增益，并明确标注“历史平均频率增益”。
- 历史值按每个音频块的实际样本数对线性增益加权，最后转换为dB；因此不受Test UI刷新率或中间界面帧丢弃影响，新session/epoch自动清零。
- L1 IMCRA与预降噪算法、L2/MUSIC与ID、L3、L4、录音和缓存均无变化；新增不等长块加权与epoch重置测试，Git LFS资产无变化。
