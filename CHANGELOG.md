...
## 2026-08-25 — 将1.3.4固定为无DOA/ID/BF实验分支

- **分支定位**：`codex/develop-v1.3.4`仅保留为实验分支，用于验证Development Test UI跳过L2 DOA、ID追踪和L3波束形成，改由Center Mic或其他L1参考音频直接进入离线L4的方案；不将其作为后续主开发基线，也不创建`v1.3.4`发布标签。
- **后续开发**：后续主开发从不可变正式基线`v1.3.3`建立`codex/develop-v1.3.5`，不继承本实验分支的旁路架构。
- **未修改范围**：本次只补充分支定位文档；L1～L6算法、Runtime、Development Test UI代码、录音与数据管理、测试、模型和Git LFS资产均无变化。
- **验证**：文档差异检查通过；文档定位变更不重新运行pytest。
