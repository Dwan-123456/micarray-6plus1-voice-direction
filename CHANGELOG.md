...
5. 本文件记录“发生了什么”；当前发布版本为`1.5.1`，完整旧架构以不可变标签`v1.3.6`为准，当前精简架构以根`README.md`、`config/config.yaml`和代码为准。

## 2026-09-04 — 正式发布v1.5.1

- **版本与标签**：以不可变`v1.4.4`为基线发布Avatar专用适配接口，形成`v1.5.1`发布提交并创建新的annotated tag `v1.5.1`；跳过`1.5.0`是用户指定的版本安排，既有标签保持原位。
- **Avatar适配**：Runtime固定MUSIC阶数不再硬编码为2，公共控制接口可设置1、2或3阶；Avatar桥接先关闭GCC-PHAT声源数估计，再把MUSIC固定为1阶。独立Mic Array默认配置继续保留声源数估计及阶数跟随能力。
- **验证与边界**：功能建设阶段聚焦测试为`104 passed`；按用户要求，发布阶段不再追加测试。尚未执行真实麦克风与Avatar端到端实机验收。
- **资产与未改范围**：L1采集、校准、IMCRA、预降噪、MUSIC数值实现、方向ID、默认配置、模型、音频、依赖和Git LFS资产均无变化；运行数据与凭据不进入发布。

## 2026-09-04 — 正式发布v1.4.4

- **版本与标签**：收束`v1.4.3`发布后的文档澄清和当前L1/L2精简实现，形成正式`v1.4.4`发布提交；创建新的annotated tag `v1.4.4`，既有发布标签保持原位且不重写。
- **发布范围**：保留MA-USB8 48 kHz/8通道采集、7物理麦校准、L1 IMCRA与P Gate、GCC-PHAT突出声源数、2–4 kHz Rolling NormMUSIC、Circular IMM-JPDA方向ID、Development Test UI、性能统计和技术文档；不包含2026-09-04开始的Avatar专用适配。
- **验证与边界**：发布前全量pytest为`165 passed`，Ruff和`pip check`通过，Git LFS对象校验通过；按用户要求不再追加环境、链接、构建或干净克隆测试。自动测试不替代新的真实麦克风实机验收，既有v1.4.3一小时实机结果继续作为历史证据。
- **资产与未改范围**：L1/L2算法参数、校准参数、模型、音频和Git LFS资产均无变化；`.venv/`、`data/`、`tmp/`、日志、缓存、凭据和本地配置不进入发布。

## 2026-09-04 — 支持Avatar关闭声源数估计并固定一阶MUSIC

- **Runtime固定阶数**：移除Runtime对固定二阶的硬编码，固定MUSIC阶数现在读取`layer2.effective_order_limit`并允许通过现有运行时接口设置为1、2或3；关闭声源数估计及阶数跟随后，后续实算和自适应复用均保持所选固定阶数。
- **Avatar集成目标**：为Avatar桥接在启动时关闭GCC-PHAT声源数估计、固定一阶MUSIC提供稳定公共控制接口；独立Development Test UI及其他调用方仍可按其配置使用声源数估计。
- **影响范围**：修改Application Runtime、声源数/阶数控制测试和变更记录；L1采集、校准、IMCRA、预降噪、MUSIC实现、方向ID、音频数据、模型、依赖和Git LFS资产均无变化；尚未进行麦克风实机验收。

## 2026-09-01 — 明确双讲话人方向追踪适用假设

- **基本假设**：第05章明确同时存在的两个讲话人在整个追踪期间必须始终保持不小于50°的圆周角距；候选rank交换仅表示两个已分离峰的排序变化。
- **范围边界**：当前方向追踪不处理双人方向重叠、相互交叉或交换方位；第01章同步把这些场景列入模型外范围，第05章测试说明同步排除相关支持与验收含义。
- **影响范围**：只修改第01、05章Markdown和变更记录；L1、L2算法、方向ID状态机、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 明确Gate连续OPEN门槛跟随MUSIC协方差长度

- **时间长度绑定**：第04章明确Gate连续OPEN门槛必须与MUSIC滚动协方差长度一致；当前`context_ms=200`对应200 ms和10个hop，后续调整为160、240或320 ms时必须同步对应为8、12或16个hop。
- **现有实现**：记录`Layer2Pipeline`已经通过`scan_config.context_ms // 20`派生所需hop数，没有独立硬编码200 ms；修改协方差长度后仍需重新验证ID出生延迟和误报。
- **影响范围**：只修改第04章Markdown和变更记录；L1、L2算法、方向ID状态机、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 补充MUSIC不采用DPD与Whitening的结论

- **历史方法结论**：第04章“历史声源数方法”补充DPD rank-1和IMCRA噪声whitening的历史测试结论：两者增加逐频计算和矩阵处理成本、压缩20 ms实时预算，现场效果不如不加时的固定频段NormMUSIC，因此当前不建议使用。
- **性能证据与版本边界**：记录历史真实麦克风对照中Whitening关闭约`4.2～6.1 ms`、开启约`6.7～15.9 ms`，并说明`v1.4.3`已删除DPD、IMCRA空间噪声协方差、Whitening配置和运行入口。
- **影响范围**：只修改第04章Markdown和变更记录；L1、L2算法、方向ID、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 补充长期会议声纹库维护方案

- **长期人物拼接**：第7.6节补充长期会议中通过会话级声纹库识别相隔较远的同人语音，并按绝对录音时间合并到统一人物时间线；明确方向`track_id`只表示空间轨迹。
- **v1.3.6历史组合**：按不可变标签代码和配置写明CAMPPlus负责16 kHz语音的192维speaker embedding，`spectralcluster==0.2.22`的`MultiStageClusterer`负责声纹证据库、增量聚类和历史标签修正；说明SpectralCluster实现Google说话人日志研究算法、由`wq2012`维护且不属于Google官方产品。
- **影响范围**：只修改第07章Markdown和变更记录；L1、L2算法、方向ID、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 精简第07章MossFormer2小节标题

- **标题文字**：按用户要求删除第7.3节标题中的“为什么”，标题改为“不能直接把Center或HardwareMix送入MossFormer2”，正文保持不变。
- **影响范围**：只修改第07章Markdown和变更记录；L1、L2算法、方向ID、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 删除第07章目标与听感说明

- **文档删除**：按用户要求删除第07章“音频还原与讲话内容可懂”中从“合理目标应表述为”开始的目标列表、评价指标说明和`v1.3.6`历史实机主观听感段落；保留本节定义与后续输入契约。
- **影响范围**：只修改第07章Markdown和变更记录；L1、L2算法、方向ID、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 调整方向ID丢失状态说明

- **README文字**：按用户要求将方向ID关键运行契约中的“最长2 s滑行”改为“最长预测2 s”，让`coasting`状态的实际行为更直接易懂。
- **影响范围**：只修改根README和变更记录；L1、L2算法、方向ID状态机、Test UI、Runtime、配置、测试、依赖、模型和Git LFS资产均无变化；已发布不可变标签`v1.4.3`保持不变。

## 2026-09-01 — 恢复项目默认上下文长度

- **Codex项目配置**：删除项目级`model_context_window=1000000`和`model_auto_compact_token_limit=900000`覆盖项，后续由当前模型与Codex运行时使用默认上下文窗口及自动压缩阈值；项目模型选择仍保持`gpt-5.6-sol`。
- **影响范围**：只调整`.codex/config.toml`和变更记录，不修改项目运行代码、算法、依赖、模型或Git LFS资产；已发布不可变标签`v1.4.3`保持不变，不运行pytest。

## 2026-09-01 — 发布后调整第07章历史音频说明

- **文字修改**：按用户要求调整WPE历史表述；第07章统一将WPE放在`v1.3.6`完整历史音频链与选择性恢复语境中。
- **音频还原目标**：第2节改为“音频还原与讲话内容可懂”，明确`v1.3.6`历史链路用于还原连续可播放语音，并使人能够识别讲话内容；“还原”表示可懂增强结果，不等同于无损恢复录音前声波。
- **Beamforming结论**：低频问题改为已完成测试的确定结论，明确DS、loaded MVDR、分频段和按空间可分性切换方法均无法有效分离低频双源，复杂方法听感不好且计算负载更高；删除研究建议并移除“可能”表述。
- **删除章节**：删除原第09节“混响和RTF”全部内容，后续STFT、按ID音频、输出契约、实时实现和参考资料章节统一前移为第9～13节。
- **版本边界**：本次只更新默认分支文档，已发布不可变标签`v1.4.3`保持不变。
- **影响范围**：L1/L2代码、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化；按用户要求不运行pytest。

## 2026-09-01 — 正式发布v1.4.3

- **版本与标签**：收束`v1.4.2`后的29个开发提交、64个变更文件，形成正式`v1.4.3`发布提交。`pyproject.toml`、根README和Test UI版本均为1.4.3；创建一次性annotated tag `v1.4.3`，既有`v1.3.6`和`v1.4.2`标签不移动、不覆盖、不重写。
- **精简运行范围**：当前主链只保留真实MA-USB8 48 kHz/8通道采集、7物理麦校准、L1 IMCRA与P1/P2、可选但不推荐的输入预降噪、20 ms P Gate、持续GCC-PHAT声源数0/1/2、2–4 kHz Rolling NormMUSIC、Circular IMM-JPDA方向ID、Development Test UI、性能统计和稀疏轨迹日志。正式录音、L3～L6、模型推理、HTTP及Production UI继续保存在不可变标签`v1.3.6`。
- **发布质量**：修复编号采集队列竞争、CDC异常重启句柄竞争、Test UI `pythonw`空退出码误报；新增克隆后双击的一键环境安装和Test UI启动入口；Python 3.12检测覆盖py launcher、当前用户目录和`Program Files`，直接依赖固定，`pytest=9.0.3`、`setuptools=83.0.0`。
- **文档**：GitHub首页提供八章技术文档导航；正文按当前代码说明阵列几何、IMCRA/P Gate、声源数、NormMUSIC、方向ID、限制、`v1.3.6`历史音频链及Runtime。用户确认的现场观察、历史方法结论和DS/loaded MVDR建议已记录，当前与历史边界分开。
- **验证**：Ruff通过；全量pytest为`165 passed`，覆盖率80%；`pip check`、Python 3.12运行环境实导入、配置加载、Test UI独立GUI进程启动、校准报告SHA-256、30个Markdown/62个本地链接、Git连通性和LFS fsck均通过。从GitHub当前HEAD干净克隆成功，9个LFS路径完整；一键安装、依赖版本、165项测试和54项干净wheel均通过。最新串口修复有模拟故障测试，未执行真实COM拔插故障注入。
- **Git LFS与公开边界**：发布树含9个LFS路径、8个唯一对象；本发布提交不新增或修改LFS对象。仓库按用户决定保持公开，本次不新增代码许可证，也不调整现有论文PDF资产。`.venv/`、`data/`、`tmp/`、录音、日志、缓存和本地构建产物未提交。
- **已接受边界**：用户接受极坐标面板保留最后结果、L2线程缺少完整端到端测试、三项UI配置未接入、未使用`BlockFanout`的竞争风险，以及许可证/PDF边界维持现状；这些不进入当前L1/L2运行主链的发布阻断。
...
- **公开边界**：README明确仓库按公开项目维护，同时保留正式标签前需要单独确认代码许可证和论文PDF再分发边界的提示；本次不替用户选择许可证，也不新增、删除或重写第三方资料。
- **影响范围**：仅修改治理与说明文档。L1/L2代码、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化，不创建或修改发布标签。

## 2026-09-01 — 合并Beamforming方法并收束为DS/MVDR建议

- **章节结构**：将第07章原第5、6、7节合并为“Beamforming方法与测试结论”，在同一节说明DS、loaded MVDR和历史复杂BF分支，后续章节编号同步前移。
- **现场结论**：记录`v1.3.6`测试结果：分频段并根据空间可分性`rho`在Dual LCMV、soft-null和loaded MVDR间切换后，听感仍不好，伪影和目标失真没有稳定改善，计算与状态复杂度反而增加。
- **推荐范围**：当前只推荐DS和loaded MVDR；LCMV、soft-null及按频段/`rho`动态切换BF仅作为历史实验保留，不建议恢复到`v1.4.3`精简实时链。同步删除后续实验顺序中的LCMV推荐。
- **内容精简**：按用户要求删除原第16～18节“评价指标、建议实验顺序、验收门槛建议”，合并后的参考资料章节前移为第14节。
- **影响范围**：仅修改解释型Markdown。L1/L2代码、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化，不创建或修改发布标签。

## 2026-09-01 — 将第07章校正为v1.3.6历史已实现音频链

- **章节定位**：第07章从“未来方案”改为“`v1.3.6`历史实现 + 选择性恢复参考”，明确旧标签已经完成L3波束形成、按ID连续音频、L4双人分离、正式录音、L5人声分类和L6声纹归并，`v1.4.3`因CPU/GPU、内存及实时调度负载而省去这些模块。WPE在旧资料中仍是研究/上界测试项，未写成已完成代码。
- **历史听感**：记录用户对`v1.3.6`实机输出的主观观察——能够听懂讲话文字；同时明确该观察没有固定语料、逐字准确率或客观音质指标，不能替代正式验收。
- **导航与恢复边界**：README导航改为“历史完整音频链”，正文把“未来L3”统一改为从`v1.3.6`选择性恢复，并要求重新验证当前L1/L2时间轴、DTO与性能预算。
- **影响范围**：仅修改解释型Markdown。L1/L2代码、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化，不创建或修改发布标签。

## 2026-09-01 — 明确精简分支未包含功能保存在v1.3.6

- **功能范围说明**：在当前局限章明确`v1.4.3`不运行的正式录音、回放、L3波束形成、L4双人分离、L5 NVIDIA人声CNN、L6声纹与人物身份、旧模型推理、HTTP服务和Production UI均保存在不可变标签`v1.3.6`，需要旧完整系统时从该标签检出。
- **双源夹角表述**：删除“推荐约90°”的工程布置建议，只保留当前算法要求的至少50°最小分离边界，并明确50°不代表能够稳定分开的准确率保证。
- **完整空间表**：补充`v1.3.6`保存的`[169,60,360]`预计算空间可分度表及生成脚本位置，说明当前热力图只用于总体趋势，逐频率和逐绝对入射方向数值应从旧标签读取。
- **影响范围**：仅修改解释型Markdown。L1/L2代码、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化，不创建或修改发布标签。

## 2026-09-01 — 补充Gate、降噪与声源数设计说明

- **P Gate与历史CNN**：说明NormMUSIC归一化会产生相对最大方向，最大峰本身不能证明真实声源存在，P Gate用于在MUSIC前过滤宽带证据不足窗口；补充`v1.3.6`的L5使用NVIDIA `Frame_VAD_Multilingual_MarbleNet_v2.0` CNN执行逐20 ms人声概率判断，并明确`v1.4.3`已移除该模型和推理依赖。
- **降噪建议**：将输入侧IMCRA-Wiener预降噪明确标为不建议开启，推荐在方向检测和目标音频提取完成后对输出音频降噪，避免改变GCC-PHAT与MUSIC依赖的原始多麦空间关系。
- **声源数稳定性**：说明Gate关闭期间持续估计用于防止Gate开启时缺少可用计数；记录后续可降低完整估计频率，或检测到2声源后短时保持MUSIC二阶以减少阶数抖动。历史CountNet结论补充为计算慢且现场效果不好。
- **影响范围**：本次只修改现行技术说明。L1/L2代码、方向ID、Test UI、Runtime、配置、依赖、模型和Git LFS资产均无变化，不创建或修改发布标签。

## 2026-09-01 — 修复CDC串口异常重启竞争

- **串口生命周期**：`SerialDevice`新增独立生命周期转换锁，启动、停止和写入操作按同一转换顺序执行。异常重启先通知旧读取线程停止并等待其退出，再关闭遗留串口和打开新连接，避免Windows OVERLAPPED串口句柄在读取期间被并发关闭。
- **超时保护**：旧读取线程在1秒内未退出时保留原线程、端口和停止事件用于后续状态诊断与再次回收；启动返回明确错误，不会在旧线程仍持有COM口时创建第二个连接。正常停止继续遵循“等待reader→关闭port”的顺序。
- **测试与边界**：新增可控串口生命周期回归，验证异常重启严格按`旧线程join→旧端口close→新端口open→新线程start`执行，并验证正常停止顺序。极坐标过期显示、L2端到端线程覆盖和三项UI配置按用户确认保持现状；L1/L2算法、Test UI显示、Runtime数据契约、模型和Git LFS资产均无变化，不修改发布标签。
- **验证**：L1现行测试连同新增的安全重启和卡死reader保护为`15 passed`；Ruff通过，全量pytest为`164 passed`，运行环境实导入及项目配置加载通过。

## 2026-09-01 — 修复校准报告身份并校正文档契约

- **校准溯源**：将`hardware_calibration_report_hash`更新为当前Git跟踪报告`L1_HARDWARE_CALIBRATION_2026-08-24.json`的实际SHA-256：`acc4eddc29f76f997305f2c5394e68ca30082bfab5ac22b39169eb63268a6bbf`。新增自动测试，直接读取配置与报告文件并验证`verified`状态和哈希一致，防止后续移动或编辑报告时静默失配。
- **ID文档边界**：按当前实现明确普通epoch变化保留session内递增计数；显式关闭再开启ID追踪会重置tracker与编号，同一session可在不同重置段复用数字ID。补充稀疏日志按数字ID汇总、跨重置分析需要人工分段的限制；方向ID实现保持不变。
- **L2快照文档边界**：README改为准确描述当前校验层级：组合DTO直接校验声源数快照身份；Gate、MUSIC和方向的同窗关系由上游Pipeline结果校验及Runtime单线程顺序组装保证。L2组合快照实现保持不变。
- **验证**：配置与报告文件的独立SHA-256复算一致；新增校准测试连同该文件原有测试为`3 passed`，Ruff通过，全量pytest为`162 passed`；运行环境实导入、项目配置加载和Test UI启动预检通过。
- **影响范围**：L1/L2算法、方向ID代码、L2组合DTO、Test UI、Runtime调度、模型和Git LFS资产均无变化；不创建、移动或改写发布标签。

## 2026-09-01 — 加固全新下载的一键安装与Test UI启动

- **安装失败检测**：`setup_vscode_env.ps1`现在逐项检查打包工具、项目依赖、editable安装、`pip check`和运行环境检查的退出码；任一步失败都会让根目录安装器返回失败，不再显示成功提示。运行环境检查强制Python 3.12，实际导入全部依赖，并加载项目配置与Test UI模块。
- **启动与停止**：`启动Test UI.cmd`在打开界面前执行完整预检，环境或配置损坏时显示错误提示；隐藏启动进程异常退出时同样显示消息框。Test UI“停止采集”按钮不再把Qt的`False`点击参数当成0秒超时，恢复使用配置中的优雅停止时限。
- **验证与边界**：新增停止按钮无参数调用回归测试。推送提交`ef26c30bfc7a128f131ef4409ae10713edcb1874`后从GitHub全新克隆并拉取9个LFS路径：禁用包索引时安装器返回1且不显示`[OK]`；回收失败环境后正常双击式安装成功，启动预检、离屏窗口生命周期和Ruff通过，全量pytest为`161 passed`。L1/L2算法、校准参数、运行时数据契约、模型、Git LFS资产和发布标签均无变化。

## 2026-09-01 — GitHub全新克隆与一键运行验收通过

- **全新下载验证**：从GitHub默认`codex/develop-v1.4.3`分支执行depth-1克隆并拉取全部9个Git LFS对象，检出提交`b8ef76f8ddc8bb72013063450981e5f8e05efec3`。在无项目虚拟环境的干净克隆中运行`一键安装环境.cmd --no-pause`，成功创建`.venv-v1.4`、安装锁定依赖、构建并安装editable包、通过`pip check`和运行环境检查。
- **启动与测试**：干净克隆中的`启动Test UI.cmd --check`成功导入UI并加载48 kHz配置；Ruff全仓检查通过，pytest为`160 passed`，Git工作树除忽略的本地虚拟环境外保持干净。用户下载后可按“克隆→双击一键安装→双击启动Test UI→开始/停止采集”使用；真实采集仍要求连接Sipeed MA-USB8和麦克风阵列。
- **影响边界**：本条只记录已完成的安装、启动和自动测试验收，不修改算法、配置、模型、Git LFS资产或发布标签。
...
- **影响边界**：仅修改Markdown文字。测试、模型、Git LFS资产和发布标签均无变化。

## 2026-09-01 — 调整IMCRA论文信息表达

- **文档表述**：将IMCRA来源改为“论文名称、作者Israel Cohen、所属Technion – Israel Institute of Technology、项目工程化版本”的顺序，提升可读性；论文文件、算法、代码和配置均无变化。
- **影响边界**：仅修改Markdown文字。测试、模型、Git LFS资产和发布标签均无变化。

## 2026-09-01 — 空间可分度表改为热力图

- **文档可视化**：将当前局限章中的空间相关度数值表替换为用户提供的6+1阵列空间可分度热力图，并用简短文字说明横轴频率、纵轴双源夹角、绿色较易分离、红色较难分离和`rho`相似度含义。
- **资产与边界**：图片保存到`docs/project_document/assets/`并由Git LFS管理。L1、L2、UI、Runtime、配置、测试、模型、论文PDF和发布标签均无变化。

## 2026-09-01 — 补充IMCRA论文作者机构

- **文档引用**：在IMCRA技术正文和参考资料索引中，将Israel Cohen的作者信息补充为Technion – Israel Institute of Technology；论文文件、算法说明和代码均无变化。
- **影响边界**：仅修改Markdown引用信息。L1、L2、UI、Runtime、配置、测试、模型、Git LFS资产和发布标签均无变化。

## 2026-09-01 — 按新版用户架构修订v1.4.3技术文档

- **README与实机结果**：将GitHub首页的类名密集架构图改写为采集整理、L1噪声/概率、160 ms窗口、L2声源数/方向/ID、发布显示五个易懂步骤，并保留类名作为代码入口。补充各阶段输入/输出/功能、默认UI操作、灯光与性能栏。按用户确认写入v1.4.3实机1小时测试结论：实时、稳定、未观察到内存泄漏，进程内存约200 MB。
- **DOA与阵列限制**：补充Gate连续OPEN约200 ms后才允许新ID出生、200 ms滚动MUSIC适合较低角速度、蓝色空间谱含义，以及SRP-PHAT双源不稳定后改用MUSIC的历史选择。按当前7麦坐标加入导向矢量相关度`rho`公式和0.5/1/2/3/4 kHz、50/90/180°计算表，说明8 cm孔径、4 cm相邻间距、离散六环麦入射方向差异、2–4 kHz选择和约700 ms端到端识别延迟。
- **未来音频链**：补充v1.3.6/v1.3.4历史建议：单人DS/MVDR，双人先按DOA做DS并按ID拼接长音频，再用MossFormer2一拆二，以1–4 kHz相似度回配方向，结合质量评分和声纹维护择优拼接；记录约10%误拆/错配及RTX 5060接近100%占用的历史观察，明确为未来方案且不得阻断当前L1/L2实时链。L1/L2代码、UI代码、运行时契约、配置、测试、模型和Git LFS资产均无变化；不创建、移动或改写发布标签。

## 2026-09-01 — 完成v1.4.3开发者详细技术文档正文

- **写作范围**：以当前代码、配置、160项测试、用户提供的文档架构和v1.4.1历史PDF为输入，完成基本假设、阵列几何与通道映射、IMCRA/预降噪/P Gate、GCC-PHAT声源数与NormMUSIC、Circular IMM-JPDA方向ID、当前局限、未来双人波束形成，以及补充的Runtime/Test UI/验证共八章正文。目标读者为具备编程能力、缺少麦克风阵列信号处理背景的技术开发者；正文加入直觉解释、公式、数组shape、参数、代码/测试入口、操作清单和验收边界。
- **总导航与事实校正**：根README补充完整端到端架构、关键运行契约、快速开始和八章超链接。修正框架阶段对MUSIC历史的错误描述：单个`DecisionWindow`携带160 ms、首次重建15帧，扫描器通过连续窗口状态扩展到200 ms/19帧。v1.4.1参考PDF中的旧FFT、P1权重、10秒音频环和历史性能数字只作为写作参考，不写成v1.4.3当前事实。
- **边界与资产**：当前局限章明确硬件CH7/Center映射复核、远场/二维/50°边界、Gate/计数/MUSIC/ID解释限制、缺少带标注真实音频与实机准确率验收。未来波束形成章明确为方案，不声明当前已实现或能够无损恢复原始音频。L1/L2代码、Development Test UI代码、录音与数据管理边界、运行时契约、配置、测试、模型和Git LFS资产均无变化；不创建、移动或改写发布标签。

## 2026-09-01 — 按用户架构建立v1.4.3详细技术文档框架

- **总导航**：根据用户提供的`文档架构.docx`，在根README建立总体架构到七个细节章节及参考资料索引的超链接导航，GitHub首页可直接进入各部分。
- **章节框架**：在`docs/project_document/`新增基本假设、阵列几何与通道映射、IMCRA/预降噪/P Gate、声源数与NormMUSIC DOA、方向ID追踪、局限与待补充、未来波束形成与双人音频恢复七个Markdown骨架；第六章按用户要求只保留空白写作位置。新增参考资料索引，并收入用户提供且已核验的Cohen 2003 IMCRA与Grondin等2022 ODAS论文原文；GCC-PHAT、MUSIC及JPDA/IMM专门来源继续预留。
- **影响边界**：本次只建立文档层级、章节标题、导航和参考资料入口，不编写完整技术正文。两份新增PDF使用Git LFS管理；L1、L2、离线L4、L5、Development Test UI、录音与数据管理、运行时契约、配置、测试、模型、算法和历史资料均无变化；不创建、移动或改写发布标签。
...
- **技术边界校正**：明确`DecisionWindow`固定160 ms/7680 samples，MUSIC实际协方差最多使用该160 ms与15个STFT帧；配置中的200 ms同时承担连续Gate OPEN 10 hop后允许ID出生的预热门槛，不代表当前窗口额外携带200 ms音频。修正缺少`source_counting`配置段时估计与MUSIC阶数跟随均默认开启的说明。
- **未改范围**：只更新现行说明文档。L1、L2、离线L4、L5、Development Test UI代码、录音与数据管理、运行时契约、配置、测试、模型、算法、历史归档和研究报告均无变化；无Git LFS对象新增或修改，不创建、移动或改写发布标签。

## 2026-09-01 — v1.4.3 项目文档目录保持纯净

- **目录边界**：移除自动生成的项目写作资料索引；项目代码、配置、测试和资产遍历结果仅保留在当前任务上下文中。`docs/project_document/`只保留根README跳转所需的占位主页，后续内容由用户的完整项目文档写作填充。
- **影响边界**：L1、L2、离线L4、L5、Development Test UI、录音与数据管理、运行时契约、配置、测试、模型、算法、历史资料和Git LFS对象均无变化；不创建、移动或改写发布标签。
...
- **项目配置**：新增`.codex/config.toml`，仅在信任并打开本仓库时选择`gpt-5.6-sol`，将可用上下文窗口设为`1,000,000 tokens`，并在`900,000 tokens`触发自动历史压缩；不修改用户级Codex配置。
- **作用范围与兼容性**：配置用于长时间代码审查、跨模块追踪和大批量文档读取，需重新打开项目任务后生效。L1～L6、Development Test UI、录音与数据管理、运行时契约、测试、模型及二进制资产均无变化；无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — 开始v1.4.3开发

- **开发基线**：从不可变标签`v1.4.2`对应发布提交建立`codex/develop-v1.4.3`，项目包版本和Test UI页脚更新为`1.4.3`。
- **代码范围**：本提交只切换后续开发版本标识；L1、L2算法、UI行为、自适应降频、配置、缓存、测试资产和外部接口均无变化。无Git LFS对象新增或修改，不创建`v1.4.3`发布标签。

## 2026-08-31 — 正式发布v1.4.2

- **发布范围**：将当前1.4精简架构正式发布为`v1.4.2`。保留L1实时采集、IMCRA/P1/P2与可选预降噪，以及L2 Gate、增量声源数估计、MUSIC DOA、IMM-JPDA/Kalman ID追踪和动态降频；L3～L6、录音管理、模拟输入及神经网络依赖继续不属于1.4运行范围。
- **发布状态**：真实麦克风复测确认20 ms周期约50 fps、0故障、0 drop，新ID能够进入`confirmed`；L1界面最新5 Hz/0.2秒平均修改通过自动化验证。发布前全量pytest为`160 passed`，全仓Ruff与`git diff --check`通过。
- **版本与资产**：项目包版本固定为`1.4.2`，创建不可变标签`v1.4.2`并推送GitHub。无Git LFS对象新增或修改；后续开发转入`codex/develop-v1.4.3`，不移动或重写该标签。

## 2026-08-31 — v1.4.2 L1概率与响度显示改为5 Hz/0.2秒平均

- **显示刷新契约**：Test UI继续以20 ms定时器消费实时邮箱并维持L2角度、ID和控制显示；L1的P1、P2、八通道响度及预降噪平均增益独立改为每200 ms刷新一次，即5 Hz。
- **0.2秒统计**：L1显示端仅保留最近10个20 ms快照。P1逐麦概率、P2阵列概率和预降噪增益采用10帧算术平均；各通道dBFS先恢复为线性功率、取10帧平均功率后再换算为dBFS，正确表示0.2秒整体RMS。session或epoch变化时清空显示历史，防止跨采集平均。
- **边界与验证**：该缓存仅存在于UI且固定最多10项，不写磁盘、不进入运行时音频缓存。专项验证覆盖10帧P1/P2、增益、功率域dBFS平均、200 ms显示周期和缓存上限。L1 IMCRA/P Gate、L2 Gate/MUSIC/ID、自适应降频、声源数估计、阵列几何和外部性能接口均无变化；L3～L6与录音管理继续保持1.4精简状态。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — v1.4.2 Test UI显示稳定性与声源数默认设置

- **Test UI表格**：L2轨迹表由按内容自动伸缩改为固定列宽，`状态`列扩展至112 px并完整容纳`coasting`；最后的`观测`列继续填充剩余空间，实时状态变化不再触发表格列宽重算和界面闪动。
- **L1电平显示**：八通道响度柱及文字显示下限由`-90/-120 dB`统一调整为`-60.0 dBFS`，低于该值的实时电平按`-60.0 dB`显示；IMCRA内部计算、P1/P2和原始采集数据均不改变。
- **默认控制状态**：声源数估计继续默认开启，`MUSIC阶数使用估计结果`改为默认开启；运行时仍允许在Test UI中手动关闭，关闭声源数估计时继续原子关闭阶数跟随并回到固定2阶。
- **验证与未改变项**：新增L1显示下限、轨迹表固定列宽及默认控制状态回归测试。L1 IMCRA/P Gate算法、L2 MUSIC与ID参数、自适应降频、阵列几何、缓存和外部接口均无变化；L3～L6、录音管理和模型资产继续保持1.4精简状态。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — v1.4.2自适应回退预算与ID确认周期修复

- **问题定位**：真实麦克风现场观察到单次L1/L2总计算约`24 ms`时，自适应控制器仍以固定`20 ms`判断每次实算，周期从20 ms连续增长到200 ms并锁定为5 fps；固定500 ms确认窗在该周期内最多容纳3次真实MUSIC观测，无法达到10次确认要求。现场同时保持50 fps复用输出、0排队、0故障和0 drop，确认问题位于回退/生命周期契约，不在采集队列。
- **动态计算预算**：队列等待继续按20 ms输出时钟判断；预降噪、IMCRA、P、声源计数、MUSIC、ID及L2总计算改为按当前实算周期判断。约24 ms负载只会从20 ms回退到40 ms，在40 ms预算内视为可持续，不再级联到200 ms。恢复判断对下一档保留固定8 ms余量，使突发升档可逐步回落到能够容纳当前负载的最小周期，同时保留基础20 ms档原有12 ms恢复门限。
- **ID确认契约**：跳过窗继续只复用最近输出，不计为新观测。ID仍需10次真实关联且existence probability达到原门限；确认时间窗取原500 ms与`(观测次数-1) × 当前实算间隔`两者较大值。因此40 ms周期仍沿用500 ms窗口，60～200 ms临时周期会扩展时间窗并允许第10次真实观测完成确认。重复轨迹合并使用同一有效窗口，tentative/coasting TTL、JPDA概率和卡尔曼参数均不变。
- **验证与未改变项**：新增24 ms负载稳定在40 ms、突发回退逐档恢复以及200 ms周期10次真实观测可确认的回归测试；全量pytest为`157 passed`，全仓Ruff和`git diff --check`通过。L1 IMCRA/P1/P2、P Gate频率权重与门限、声源计数算法、MUSIC谱和峰值参数、阵列几何、UI布局、缓存、录音/数据管理、L3～L6删除状态及模型资产均无变化；修复后真实麦克风复测将在提交并重启Test UI后执行。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — v1.4.2 P Gate采用30%/30%/40%宽带外层与低段统计细分

- **涉及文件**：`layer1_input/speech_spectrum.py`、`layer1_input/README.md`、`layer1_input/algorithms/README.md`、`common/config.py`、`config/config.yaml`、`tests/test_l1_v03.py`、`tests/test_l1_pre_denoise.py`及本变更日志。
- **二层权重**：P1外层频率范围恢复为250～3400 Hz，并按250～600、600～1600、1600～3400 Hz三段固定分配`30%/30%/40%`。600 Hz归第一段，第二段在50 Hz离散频轴上使用650～1550 Hz，避免边界重复。第一段的30%继续按上一版办公室与诊室统计曲线在250、300、350、400、450、500、550、600 Hz八个bin内细分，内部权重为`0.161502/0.180000/0.180000/0.180000/0.163814/0.088159/0.041165/0.005360`，对应全局权重为`0.0484506/0.054/0.054/0.054/0.0491442/0.0264477/0.0123495/0.001608`。第二段的19个bin段内等权，每bin约为`0.0157895`；1600～3400 Hz的37个bin段内等权，每bin约为`0.0108108`。
- **宽带边界**：三段总质量严格为`0.30/0.30/0.40`且全频总和为1。任意两个频段即使SPP全部为1，总贡献上限也只有`0.60`或`0.70`，均低于Gate门限`0.80`；250 Hz间隔稀疏谐波及其相邻50 Hz泄漏的合成SPP数组也必须低于门限。该约束保留250～600 Hz统计排序，同时要求三个宽频段共同提供人声证据。
- **配置与身份**：`layer1_imcra.frequency_max_hz`及严格配置模型由`600.0`恢复为`3400.0`，算法身份由`cohen_imcra_2003_l1_v10`升级为`cohen_imcra_2003_l1_v11`。P1仍按逐麦SPP加权，P2仍取7个物理麦P1中位数；L2 Gate的`P2 >= 0.80`比较、20 ms周期、960点FFT、IMCRA状态与SPP计算均保持不变。
- **验证边界**：L1专项测试为`13 passed`，全量pytest为`155 passed`，全仓Ruff和`git diff --check`通过。验证覆盖64个50 Hz Gate bin、三段质量、低段八点细分、600 Hz边界归属、650 Hz进入第二段、任意两段饱和上界、稀疏谐波及均匀宽带SPP；按用户要求不使用模拟音频或本地录音结果作为本次验收依据，尚未完成真实现场复测。
- **未改变项与资产**：IMCRA噪声递归、可选Wiener预降噪、HardwareMix、声源计数、MUSIC/IMM-JPDA、阵列几何、UI、运行时队列与1秒缓存、L3～L6删除状态、录音/数据管理和模型均无变化。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — v1.4.2 P Gate改用办公室与诊室统一50 Hz逐bin权重

- **涉及文件**：`layer1_input/speech_spectrum.py`、`layer1_input/README.md`、`layer1_input/algorithms/README.md`、`common/config.py`、`config/config.yaml`、`tests/test_l1_v03.py`、`tests/test_l1_pre_denoise.py`及本变更日志。
- **L1/P Gate频谱参数**：将原250～3400 Hz三段15%/35%/50%聚合替换为250～600 Hz共8个50 Hz bin；250、300、350、400、450、500、550、600 Hz权重依次为`0.161502/0.180000/0.180000/0.180000/0.163814/0.088159/0.041165/0.005360`，总和严格为1。运行时对频率轴执行精确契约校验，缺少bin、额外bin或步长不符时立即拒绝启动，避免配置范围和权重表静默错位。
- **设计依据与适用边界**：统一权重取办公室和诊室两套稳健频谱方案的逐bin算术平均，并保持单bin最高18%的上限。统计覆盖办公室/会议室常见空调与HVAC、电脑风扇、键盘、鼠标点击及摩擦代理、脚步、桌椅、纸张、打印机和门等噪声，也覆盖诊室的通风、风扇、设备机械声、门、纸张和一般室内活动声；按需求排除咳嗽和医疗报警。当前参数来自公开语音长期平均谱、公开环境音频库统计与代理类别，尚未用本机阵列的办公室/诊室标注录音完成ROC标定。
- **配置与接口**：`layer1_imcra.frequency_max_hz`及其严格配置模型由`3400.0`改为`600.0`，算法身份由`cohen_imcra_2003_l1_v9`升级为`cohen_imcra_2003_l1_v10`以区分新旧P1语义。`frequency_min_hz=250.0`、960点FFT、50 Hz频率步长、20 ms更新周期、IMCRA状态与SPP计算均保持不变。P1继续计算逐麦加权SPP，P2继续取7个物理麦P1中位数；L2 Gate比较规则与默认门限`0.80`保持不变，真实阵列数据采集后仍需单独复核该门限。
- **文档与验证**：更新L1当前说明并同步修正同一行已过时的10秒音频缓存描述为当前1秒。针对性L1测试为`12 passed`，全量pytest为`154 passed`，全仓Ruff和`git diff --check`通过。自动测试确认8个频点、逐bin权重、权重总和及错误频率轴拒绝行为；本次未进行真实办公室或诊室硬件验收。
- **未改变项与资产**：可选Wiener预降噪、HardwareMix、声源计数、MUSIC/IMM-JPDA、阵列几何、UI、运行时队列与缓存实现、L3～L6删除状态、录音/数据管理和模型均无变化。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — v1.4.2实时回退、ID寿命与诊断稳定性修复

- **自适应回退**：L2过载降频现在连同GCC-PHAT声源数、MUSIC特征分解/峰值搜索和ID更新一起按动态周期执行，并将上一窗的声源数、Gate与方向结果按当前20 ms时间戳重新发布。每个跳过窗仍只维护轻量滚动MUSIC协方差，确保下一次实算继续使用完整200 ms统计历史；该耗时继续纳入MUSIC过载观测。人数统计把控制器计划的40～200 ms整数间隔视为连续实算观测，保留2-of-3稳定投票；真实非计划丢窗仍清空投票。L2 worker按顺序消费所有未溢出窗口，队列满时沿用既有策略只淘汰一个最旧窗并累计`processing_drops`。
- **TTL与连续性**：自适应复用和L2异常复用均按原配置的tentative 500 ms、confirmed/coasting 2000 ms寿命过滤ID，达到TTL后清除旧ID方向与对应候选，持续异常不能无限续命；最近MUSIC谱只作为标记过复用状态的诊断背景保留。Gate活跃计数只接受严格相邻的960 samples窗口；真实丢窗会把新ID预热重新计为1，动态降频仍因Gate每20 ms独立评估而保持连续预热。
- **ID开关与诊断接口**：关闭ID会递增独立清空代次，即使UI在一个L2窗之间完成“关→开”，下一窗仍强制清空旧ID并从新轨迹开始。`RollingNormMusicScanner.model_order`现在从初始化到reset均安全返回`None`，成功扫描后返回最近阶数。CPU基准改用当前7680 samples上下文和真实连续滑窗。
- **轨迹日志与性能显示**：轨迹TXT移至独立低优先级线程，最新任务邮箱容量1，最多保留64个历史ID及每ID 240个五秒轨迹点；写入使用临时文件原子替换，路径/磁盘异常只写入`track_log_last_error`，不再结束L2线程。生产者和消费者均按`session_id + stream_epoch`识别数据流，变化或sample轴回退时会解除1秒节流并清空上一数据流轨迹；即使新旧epoch首个sample相同，或携带重置标记的队列项被最新任务覆盖，也不会混合同号ID。默认关闭仍保持可即时切换的预降噪热备，其实际耗时现纳入1秒性能快照、总耗时、L2过载原因及Test UI底栏。
- **长时故障边界**：采集健康事件去重只保留最近1024个event ID，epoch不连续诊断只保留最近256条；累计错误仍通过既有计数器观察，12小时故障风暴不会持续扩大这两个容器。
- **验证与未改变项**：新增TTL到期、快速ID开关、Gate丢窗、MUSIC属性、日志限长、跨epoch隔离及I/O故障隔离测试；全量pytest为`154 passed`，全仓Ruff与`git diff --check`通过，修复后的50窗CPU MUSIC基准平均`0.823 ms/窗`。IMCRA/P1/P2、Gate门限、GCC-PHAT与MUSIC/IMM-JPDA数学参数、阵列几何、UI主体布局、1秒音频环、L3～L6删除状态、录音/数据管理和模型资产均无变化；无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — 声源数与L2结果改为同窗原子快照

- **原子发布契约**：`L2DevUiSnapshot`新增可选`source_count_snapshot`引用，并在DTO构造时强制其session、epoch、window ID和decision sample与L2完全一致；正常实算、自适应复用及已有Gate结果的L2故障回退均携带当前窗计数。独立`latest_source_count`容量1邮箱继续发布以兼容既有消费者，但不再用于Test UI跨邮箱拼接。
- **UI一致性**：Test UI只从最新L2组合快照更新突出声源数，计数故障由组合快照中的预热/不可用值表达，不再读取可能已经推进到相邻窗口的独立邮箱或全局计数错误状态。因此后台恰在两次UI读取之间推进时，也不会显示“计数N+1 + MUSIC N”。
- **验证**：新增跨窗计数DTO拒绝、复用路径携带当前计数、UI忽略竞态独立邮箱以及原子预热/故障/过期/重启回归；全量pytest为`138 passed`，全仓Ruff和`git diff --check`通过。当前`MicArray`单线程BLAS、阶数立即跟随的20秒短测处理`991`个L2/计数窗口，输出50 Hz，0丢弃、0溢出、0不连续、0错误；探针观察到的8个Gate OPEN映射全部匹配，`unjoined=0`、错误映射0，总阶段平均/P95/最大为`2.544/2.766/2.810 ms`。该短测验证发布关联和即时实时性，不替代20分钟稳定性及带真实标签的准确率测试。
- **未改变项**：GCC-PHAT/de-emphasis、IMCRA、P Gate、MUSIC/IMM-JPDA数学、声源数到MUSIC阶数规则、自适应周期、缓存/队列容量、UI布局和性能栏均无变化；组合快照只增加一个已有不可变对象引用，不复制音频或频谱。L3～L6删除状态、离线L4、L5、录音/数据管理、模型和资产无变化；无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — 实时进程固定单线程OpenBLAS/OMP并消除约1 GiB私有提交

- **进程级线程约束**：`app`包在导入Runtime及NumPy/SciPy前强制设置`OPENBLAS_NUM_THREADS=1`和`OMP_NUM_THREADS=1`；PowerShell Test UI启动器、VS Code Test UI launch/task也显式设置相同的子进程环境，不修改Windows全局环境。外部继承的更大线程数会被项目实时入口覆盖，避免OpenBLAS 24线程池为7×7小矩阵工作负载预留大块私有提交和引入调度抖动。
- **内存归因与实测**：独立进程探针中，默认线程下导入NumPy/SciPy的私有内存为`1033.8 MiB`，单线程后为`69.2 MiB`。配合1秒音频缓存的当前真实`MicArray` 20秒短测，第19秒进程私有内存为`137.434 MiB`、RSS为`143.059 MiB`；此前同一1秒缓存但默认BLAS线程的第19秒为`1101.793/145.676 MiB`，私有提交减少约`964.36 MiB`，RSS降低约`2.62 MiB`，符合大部分原空间未驻留物理内存的预期。
- **实时短测**：20秒接收`960,000`采样点并处理`991`个L2/计数窗口，输出与计数维持50 Hz，采集状态、输入溢出、handoff/处理丢弃、不连续和运行错误均为0；IMCRA、声源计数及总阶段平均耗时分别为`1.240/1.024/2.475 ms`，总阶段P95/最大为`2.854/3.222 ms`，未触发回退。该短测确认即时实时性，不能单独替代20分钟稳定性验证。
- **验证与未改变项**：新增干净子进程回归，验证即使外部指定8线程，`app`入口也在Runtime导入前收敛为1线程；全量pytest、Ruff、VS Code JSON和Git差异检查通过。线程约束不改变FFT长度、7×7矩阵维度、GCC-PHAT/de-emphasis、IMCRA、P Gate、MUSIC/IMM-JPDA、声源数映射、缓存/队列容量、UI布局或输出契约；仅可能产生符合浮点运算规则的末位舍入差异。L3～L6删除状态、离线L4、L5、录音/数据管理、模型和资产无变化；无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — 完整块与底层原始音频缓存统一缩短为1秒

- **完整块缓存**：Application Runtime的完整`IngestedAudioBlock`内存环由固定`500`个20 ms块（10秒）缩短为按采样率/块长推导的`50`块（1秒），同步更名为`_audio_cache_1s`。缓存仍在每次采集开始时清空、从不序列化；WindowAssembler的160 ms声源计数上下文、200 ms MUSIC上下文、P Gate、UI latest-only邮箱和实时处理顺序不依赖该历史环，因此算法输入与发布节拍不变。
- **底层原始缓存**：AudioCapture的原生S16 recent环由5秒缩短为1秒，即48 kHz/960帧配置下由250块降为50块。既有`latest()`接口本来就最多返回50块，`status()`只读取最新一块，因此对外可观察行为不变；设备回调、handoff队列和连续性事件契约不变。
- **实机短测**：当前`MicArray`（Windows WDM-KS，8通道，48 kHz）连续20秒接收`960,000`采样点并处理`991`个L2/计数窗口；输出与声源计数稳定为50 Hz，采集状态、输入溢出、handoff/处理丢弃、时间轴不连续和运行错误均为0。完整块环稳定封顶50块，主音频数组为`1.465 MiB`；进程RSS在第19秒为`145.676 MiB`、短测最大`146.797 MiB`，对比修改前同条件19秒约`205.047 MiB`下降约`59.37 MiB`。该短测验证容量和即时实时性，不替代此前20分钟稳定性验证。
- **验证与未改变项**：新增Runtime完整块环和AudioCapture原始环均为50块的回归；全量pytest、Ruff及Git差异检查通过。GCC-PHAT/de-emphasis、IMCRA、P Gate、MUSIC/IMM-JPDA、声源数到MUSIC阶数映射、Test UI布局、队列容量、L3～L6删除状态、离线L4、L5、录音/数据管理、模型及资产均无变化；`docs/V1.4.1_ARCHITECTURE_USAGE_AND_ONE_HOUR_TEST.md`保留其对应旧版本的10秒历史事实。无Git LFS对象新增或修改，不创建或移动发布标签。

## 2026-08-31 — MUSIC阶数控制与持续声源计数解耦及20分钟实机验证

- **控制隔离**：切换“MUSIC阶数使用估计结果”现在只改变MUSIC有效阶数选择，不再递增声源计数控制版本或重置GCC-PHAT累计/稳定状态；手动关闭声源计数仍是唯一会停止并清空计数状态的控制。新增跨连续窗口切换跟随模式的回归，确认计数器只初始化一次、连续消费新增窗口，且切换后仅把同窗计数映射为MUSIC 1/2阶。
- **真实麦克风20分钟验证**：使用当前`MicArray`（Windows WDM-KS，8通道，48 kHz）连续采集`1200 s`，共接收`57,600,000`采样点并发布`59,991`个声源计数窗口；采集回调状态、输入溢出、subscriber/handoff丢弃、Runtime处理丢弃、时间轴不连续和运行/计数错误均为`0`，输出与计数稳定为`50 Hz`。可关联的Gate OPEN跟随窗口中阶数映射`1,125`次全部匹配、`0`次错误；队列异步观测顺序造成`81`窗无法在探针侧关联，不代表运行时同窗契约失败。
- **耗时与延迟**：IMCRA平均/P95/最大为`1.466/1.665/1.987 ms`，概率为`0.109/0.121/0.155 ms`，声源计数为`0.989/1.153/1.560 ms`，MUSIC为`0.056/0.390/1.397 ms`，ID为`0.052/0.219/0.596 ms`；全阶段总耗时为`2.671/3.210/5.011 ms`，队列等待为`0.069/0.320/0.960 ms`。按设备采样时间轴估算的采集至L2发布延迟中位/P95/最大为`20/39/62 ms`；160 ms计数滚动窗和200 ms MUSIC滚动窗属于算法上下文，不是额外阻塞等待。
- **回退与资源稳定性**：受控回退从`20→60→40→20 ms`并在`6.188 s`内恢复，期间输出保持50 Hz，跳过的实算窗由既有结果复用；另有短时自然回退均自动恢复。10秒音频缓存固定封顶`500`块/`14.648 MiB`，线程数恒定`4`，句柄由`497`降至`487`；缓存填满后的RSS约从`205.22`到`208.02 MiB`、私有内存从`1162.23`到`1164.56 MiB`。20分钟内未发现无界缓存、线程、句柄或错误增长；该时长只能表明未观察到结构性泄漏，不能替代更长时内存证明。测试仅统计突出声源数与运行契约，没有房间内0/1/2真实声源标签，因此不评价计数准确率。
- **验证与未改变项**：全量pytest为`134 passed`，全仓Ruff及`git diff --check`通过；实机探针及数值结果保留在忽略的`tmp/`且不提交音频或运行数据。GCC-PHAT/de-emphasis数学和阈值、IMCRA/P Gate、MUSIC谱与IMM-JPDA数学、Test UI布局、L3～L6删除状态、离线L4、L5、录音/数据管理、模型和资产均无变化；无Git LFS对象新增或修改，不创建或移动发布标签，已发布`v1.4.1`保持不变。
