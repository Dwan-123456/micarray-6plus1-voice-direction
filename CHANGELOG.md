...
- **拼接与输出过滤**：TrackAudioStream从临时ID首次观测开始建立权威时间线并拼接L3音频；Test UI在临时阶段隐藏该音轨，待同一ID正式确认时从Hub完整连续波形回填开头。始终未确认的临时ID不会进入最终精确ID允许列表；捕获结束后仍按既有规则物理删除短于2秒或静音占比不合格的音频，离线L4/L5不会收到这些候选。
