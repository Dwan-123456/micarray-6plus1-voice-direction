...
5. 本文件记录“发生了什么”；当前1.2.2架构以`ARCHITECTURE_V1.1_TARGET.md`为权威契约，已发布1.0.1历史以`ARCHITECTURE_V0.3_TARGET.md`为基线，实际参数以`config/config.yaml`和代码为准。
...
- **L2**：Rolling NormMUSIC按手动阶数搜索候选，圆周候选最小间隔调整为50°；DPD改为按频率投票聚类；确认门限调整为6次观测；公共ID、Kalman和最多3方向契约保持不变。
...
- **测试**：发布提交全量pytest为`446 passed`，核心源码与测试Ruff全部通过，全项目Python `compileall`通过。自动测试不替代真实阵列、诊室多声源和长时间运行验收。
