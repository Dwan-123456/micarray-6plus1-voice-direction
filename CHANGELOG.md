...
- 保留为维持绝对时间轴而插入的coasting/补洞等时静音，但不再将这些人工占位静音计入“连续长静音”和“声音占比”过滤依据，避免模拟输入结束后误删已含有效L3声音的缓存。
- 真实L3静音结果仍按原规则过滤；Center Mic、20 ms稳定hop、补洞长度、交叉淡化、L2权威ID键和关窗删除行为不变。
- 新增“有效声音后长时间coasting”回归测试，验证时间轴仍等长且封存后可生成播放快照；Git LFS资产无变化。
