...
`track_id`只表示空间方向轨迹，不是人物或声纹身份。只有已达到tracking `confirmed`且至少收到一次L5正向人声反馈、当前未被标记为噪声干扰的ID，才能在正式Gate概率低于门限时强制放行MUSIC。尚无人声证据的临时轨迹有当前观测时仍可进入L3/L5接受分类，但漏检后只在`active_tracks`中保留，不再作为公共coasting方向送入L3，也不能强制Gate；最后一个符合条件的人声ID消失后，Gate立即恢复按概率门限判断。ID仍使用3秒绝对sample TTL；预热、缺失或无效概率不会被伪造成有效概率。短漏检保留同一内部ID，超过TTL后再次观察会获得新ID。epoch会清除活动轨迹，但同一session的ID计数继续递增；新session建立新的ID命名空间。
