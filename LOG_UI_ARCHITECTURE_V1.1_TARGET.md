# 6+1 麦克风阵列项目1.2.1：独立Pipeline Log UI架构

状态：**代码、公共只读查询、五个页面与自动化测试已纳入项目1.2.1；真实封存session人工回放和大规模实机数据验收仍需继续。**

发布版本：项目`1.2.1`。本文件定义Log UI的定位、只读边界、数据覆盖、页面与验收要求；未通过的实机项目必须继续明确标注。
...
- 1.2.1通过公共接口提供session decisions、track timeline、track audio assets和session audio assets等只读查询能力。
...
1. **P0：观察矩阵冻结**。确定字段 → 公开接口 → 页面 → 缺失行为；明确历史记录与1.2.1 capability。
...
Log UI作为1.2.1已实现组件满足以下自动化与接口条件：
