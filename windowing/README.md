...
本层把连续`IngestedAudioBlock [N,8]`组装为每20 ms一个只读`DecisionWindow [7680,8]`。每个窗口保留160 ms完整上下文；`physical_samples`固定为`[7680,7]`，`physical_history(160)`只返回物理麦，超出窗口的请求会被拒绝。HardwareMix不得进入MUSIC。完整8通道窗口继续供下游和记录契约使用。
