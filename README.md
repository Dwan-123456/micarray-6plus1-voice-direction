...
> 当前开发版本：`1.3.4`；Layer 2公开版本：`1.1`。最近正式发布基线为不可变标签`v1.3.3`。

> **发布状态：项目 `1.3.4`开发线。** 最近正式版已收束为不可变标签`v1.3.3`；自动化验收不替代真实双人录音、长时间运行和GPU质量门禁。

> **实验分支说明：`1.3.4`仅用于验证“不运行DOA、ID追踪与L3波束形成，改由Center Mic/参考麦音频直接进入离线L4”的方案，不作为后续主开发基线。后续正式开发从`v1.3.3`另建`1.3.5`开发线。**
...
离线L4/L5/L6不在20 ms Runtime中执行。L2按权威ID先登记绝对20 ms时间槽，L3只把BF波形填入对应槽；未生成BF的槽保留等时静音，因此不同L3算法只改变内容而不改变轨长。停止采集并排空L3后，`TrackAudioStreamHub.seal()`直接封存完整ID长音频和逐窗L2方向数量。完整模拟输入EOF等待全部阶段数据完整排空，实时/手动停止才使用有限安全超时。Test UI的L4工作流固定不合并，不再提供“合并”开关：两人轨的两条原生16 kHz候选按1～4 kHz复频谱相干度降序标记A/B，两条都进入L5；单人轨保留唯一旁路输出。每条L4波形在峰值保护后运行DNSMOS，原始SIG/BAK/OVRL与按25%/25%/50%合成的0～1 MOS写入元数据。整批L4完成后自动运行NVIDIA长音频Frame-VAD，每个20 ms hop的Voice概率在L4音频条上标黄。每批L4/L5成功完成后立即自动运行L6并更新人物声纹结果；同一封存测试用例重复发送L4时，L4/L5/L6三层均按新批次重新运行并替换旧展示。
