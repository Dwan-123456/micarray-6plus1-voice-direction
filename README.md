...
系统使用一块由6个均匀分布在半径4 cm圆周上的外圈麦克风和1个中央麦克风组成的阵列。根据当前诊室使用目的和阵列物理限制，系统只能处理同时至多2个讲话人，而且两个讲话方向的水平夹角必须大于或等于45°。系统持续完成以下工作：

1. 采集并校准多通道音频；
2. 判断当前窗口是否存在值得定位的声源；
3. 扫描阵列周围360°，找出最多两个、彼此至少相隔45°的候选声源方向；
4. 对每个候选方向增强音频；
5. 判断该方向的增强音频是否为人声；
...
- L2选择2000～4000 Hz做SRP-PHAT定位，是为了使用相对更有方向信息的频段；代价是对主要能量落在该频段之外、频谱异常或窄带的特殊声源识别与定位能力较差；
    ...
    LogicalAudio [N,8]：MIC0..MIC5、Center、HardwareMix
        ...
        └── HardwareMix [N]
              只用于接口、显示、录制和后续实验，不进入阵列几何
                        ↓
    【已完成】7麦IMCRA噪声分析
        ├── 每20 ms更新80～8000 Hz噪声PSD、SPP及诊断特征
...
Layer 2：二维声源方向定位
    【已完成】Probability Gate
        ...
        ├── 低于阈值：不进入srp
                        ↓ Gate开启
    【已完成】2000～4000 Hz SRP-PHAT 360°扫描
        ↓ Robust-Z + Sigmoid归一化
        ↓ 圆周局部峰值
        ↓ 最多2个候选方向 CandidateDirection[0..2]，两方向夹角≥45°
        ├──输出有声源概率
                        ↓
    【已完成】可选私有方向ID追踪（默认关闭）
        ├── circular_id_tracker_v4按20°圆周距离关联跨窗候选
        ├── 临时ID观察满3秒、自然Gate匹配≥5次且获得L4人声证据后转正
        └── 正式ID具有3秒存活时间，只有后续获得唯一匹配的L4人声可续期，单次续期3s
                        ↓
    【已完成】可选按ID圆周卡尔曼滤波（默认关闭，依赖ID追踪）
        ├── 平滑角度和角速度
        ├── Q/R倍率可在Test UI调节
                        ↓ 输出最多2个候选角度 → 有界L3 latest-wins队列
Layer 3：按候选方向增强音频（BF）
    输入：同一320 ms LogicalAudio + 0～2个候选角度
    当前可用：
        ├── optimized：支持最多2个声源；IMCRA空间分离，按条件使用Dual LCMV、MVDR或DAS回退
...
结果汇总与保存
    ...
    【已完成】按采集时间顺序保存，避免跨窗口并行造成结果错位
        ...
        └── Development Test UI
              ...
              └── 本机试听sidecar：Center参考 + 正式方向ID连续音频缓存
...
设备原生输出8通道。Layer 1先把它们整理成统一的逻辑顺序：`MIC0～MIC5、Center、HardwareMix`。前7路具有明确的物理位置，构成实际阵列；HardwareMix没有空间坐标，是阵列自己合成的。不参加SRP-PHAT或波束形成。
...
- 80～8000 Hz：用于噪声统计、预降噪、Layer 3增强和Layer 4分析；
- 500～4000 Hz：聚合成Layer 2的声源Gate概率；
- 2000～4000 Hz：用于SRP-PHAT方向定位。
...
Layer 2先计算窗口末尾两个连续20 ms声源概率的平均值。平均值达到默认阈值`0.60`时才运行SRP-PHAT；低于阈值时跳过定位，减少持续噪声造成的无意义方向结果和计算开销。

Gate阈值与SRP峰值阈值是两个不同参数：前者决定“是否定位”，后者决定“空间响应上的峰是否足够可信”。

### 5. SRP-PHAT二维定位

SRP-PHAT比较7个物理麦克风之间的相位时延，只使用2000～4000 Hz频率，在0～359°逐度计算空间响应。响应经过Normalization后寻找圆周局部峰，再执行45°圆周非极大值抑制，最终输出0～2个候选方向。
...
Layer 3对每个候选方向生成一条320 ms、48 kHz单声道增强音频。当前可用的优化模式支持最多2个声源，使用IMCRA统计和阵列空间信息，在Dual LCMV、加载MVDR和DAS之间选择或回退。DAS基线当前只按单声源方法使用。界面和代码中还保留固定30°恒定波束宽度实验入口，但它目前不属于可用方法，不能写入正式能力结论。
...
L4结果可以作为L2正式方向ID的“人声证据”，但反馈只包含时间身份、角度和人声结论，不传递私有ID。
...
7. 在左下角试听Center参考和已正式确认的方向音频，在右下角观察CNN人声概率。
...
- 方向轨只有在L2正式ID确认后才开始缓存，一般至少开始采集5秒后显示；
- 候选消失后等待3秒再结束对应方向轨；
- 可通过按键切换bf模型，当前可用的是`optimized`和`ds_baseline`：优化方法支持最多2声源，DAS只用于单声源；
...
- L2 Probability Gate、SRP-PHAT、私有ID追踪和圆周卡尔曼；
