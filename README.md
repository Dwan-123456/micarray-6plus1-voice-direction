...
v1.5.1 是实时方向定位精简版，并为 Avatar 阵列输入桥接提供关闭声源数估计、固定一阶 MUSIC 的运行时控制接口。完整L1～L6、录音管理、模型和示例音频保存在不可变标签`v1.3.6`。

本仓库按公开GitHub项目维护，公开源以`Dwan-123456/micarray-6plus1-voice-direction`为准。`v1.5.1`保持现有公开范围；本次发布不新增代码许可证，也不调整仓库内论文PDF资产。

## 文档导航

本文提供v1.5.1的总体架构。各部分的算法原理、输入输出、配置参数、代码位置和使用边界通过以下链接进入详细文档。
...
`v1.5.1`标签固定本次发布内容。`git lfs pull`用于下载文档中的论文和图片。
