...
【已完成】Layer 4：采集后1/2人语音分离与主候选匹配
    ...
    └── 2人：MossFormer2或TIGER → 两条匿名候选
          ...
          合并关闭：不匹配，在L4显示原ID/角度下的A/B两条16 kHz候选，并分别进入L5
...
离线L4/L5不在20 ms Runtime中执行。L2按权威ID先登记绝对20 ms时间槽，L3只把BF波形填入对应槽；未生成BF的槽保留等时静音，因此不同L3算法只改变内容而不改变轨长。停止采集并排空L3后，`TrackAudioStreamHub.seal()`直接封存完整ID长音频和逐窗L2方向数量。完整模拟输入EOF等待全部阶段数据完整排空，实时/手动停止才使用有限安全超时。Test UI下半区按L3/L4/L5三等分：“合并”默认开启（绿色），由“发送到L4”运行人数路由、分离和1～4 kHz复频谱相干匹配；短于2秒或匹配低可信时安全回退原L3人声，并保存带原ID/角度的单条L4 WAV供试听。“合并”关闭（灰色）时不运行匹配，按原ID/角度显示A/B两条16 kHz匿名候选供对比试听。整批L4完成后自动运行NVIDIA长音频Frame-VAD；关闭合并时A/B候选分别进入同一个L5。L5为每条L4音频的每个20 ms hop输出独立概率和Voice判断，黄色逐帧绘制在对应L4音频条，L3音频不再着色。
