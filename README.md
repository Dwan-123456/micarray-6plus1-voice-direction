...
> 当前开发版本：`1.4.1`；Layer 2公开版本：`1.1`。最近正式发布基线为不可变标签`v1.3.6`。

> **开发状态：项目 `1.4.1`。** 本开发线从不可变标签`v1.3.6`建立，后续修改统一进入`codex/develop-v1.4.1`；`codex/develop-v1.3.4`已停止开发并仅保留为历史实验分支。
...
- `0°`：从灯面正上方向下观察时的正右方，也是Center→MIC0方向；
- `90°`：正上方；
- 角度沿逆时针增加；
- 角度范围为`[0°, 360°)`。

外圈麦克风的固定角度映射为：`MIC0=0°、MIC5=60°、MIC4=120°、MIC3=180°、MIC2=240°、MIC1=300°`。Host通道顺序仍为MIC0～MIC5，不因几何方向改变而重排。
...
【已完成】Layer 1：输入、通道整理和噪声分析
    ...
    │     从250～3400 Hz按三频带15%/35%/50%聚合Gate概率p20 ∈ [0,1]
...
上图描述正式1.3.5代码实现；`【已完成】`表示模块和自动化契约已经接通，不代表真实阵列、诊室声场、中文目标域或长时间负载已经验收。独立Pipeline Log UI的详细只读边界见[`LOG_UI_ARCHITECTURE_V1.1_TARGET.md`](LOG_UI_ARCHITECTURE_V1.1_TARGET.md)。
...
整个项目只使用一套灯面朝上的坐标。几何、定位、波束形成、界面角度和录音manifest必须引用同一坐标，避免出现镜像、顺逆时针相反或固定角度偏移。
...
- 250～700、700～1600、1600～3400 Hz：分别先求频带内SPP均值，再按15%、35%、50%聚合成Layer 2声源Gate概率；
...
Layer 2只使用窗口末尾当前20 ms声源概率，达到默认阈值`0.60`才运行MUSIC。L2内部仍保留按精确ID接收在线人声反馈后强制放行的兼容接口，但正式L5和渐进preview都不回传L2，因此普通1.3.5运行不会触发该强制放行。预热、缺失或无效概率同样保持阻断。
