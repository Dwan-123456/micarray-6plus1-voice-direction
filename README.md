...
7. 在用户手动触发时，对L4整条A轨和满足门限的B轨提取声纹，聚为最多三人，按MOS择优拼接并压缩长静音。
...
【已完成】Layer 2 1.1：二维声源方向定位与公共方向轨迹
    Probability Gate
        ...
        当前L5仅离线运行且不回传L2，因此普通Runtime没有语义强制放行
...
【已完成】按权威ID封存离线输入
    ...
    ↓ Test UI“发送到L4”（可重复提交并选择模型）
【已完成】Layer 4：采集后1/2人语音分离与主候选匹配
    48→16 kHz；人数=min(2, 整轨L2方向数最大值)
    ├── 1人：直接旁路为L4输出
    └── 2人：MossFormer2或TIGER → 两条匿名候选
          30秒分块/1秒重叠、块间排列修复与交叉淡化
          合并开启：1～4 kHz复频谱相干匹配 → 保留原track_id和theta_deg的单条主输出
          合并关闭：保留两条候选，按匹配度降序标记A/B（A高、B低），显示原ID/A-B及分数并分别进入L5
          短于2秒、最高分<0.50或分差<0.025时安全回退原L3参考
...
离线L4/L5不在20 ms Runtime中执行。L2按权威ID先登记绝对20 ms时间槽，L3只把BF波形填入对应槽；未生成BF的槽保留等时静音，因此不同L3算法只改变内容而不改变轨长。停止采集并排空L3后，`TrackAudioStreamHub.seal()`直接封存完整ID长音频和逐窗L2方向数量。完整模拟输入EOF等待全部阶段数据完整排空，实时/手动停止才使用有限安全超时。Test UI下半区按L3/L4/L5三等分：“合并”默认开启（绿色），由“发送到L4”运行人数路由、分离和1～4 kHz复频谱相干匹配；短于2秒或匹配低可信时安全回退原L3人声，并保存带原ID/角度的单条L4 WAV供试听。“合并”关闭（灰色）时同样计算两条候选相对L3参考的匹配度，但只用于排序而不丢弃候选：A为高分、B为低分，界面显示原ID、A/B及三位小数分数，不显示角度。每条最终L4波形在峰值保护后运行DNSMOS，原始SIG/BAK/OVRL与按25%/25%/50%合成的0～1 MOS写入元数据；预览标签末尾显示三位小数MOS。整批L4完成后自动运行NVIDIA长音频Frame-VAD；关闭合并时A/B候选分别进入同一个L5。L5为每条L4音频的每个20 ms hop输出独立概率和Voice判断，黄色逐帧绘制在对应L4音频条，L3音频不再着色。
...
当前已知误检是笔记本电脑风扇：当麦克风阵列通过约50 cm连接线放在笔记本电脑附近、现场只有一个主要人声声源时，散热风扇仍可能形成一个稳定候选方向。现有Gate和MUSIC只能说明某个方向存在符合频带和空间特征的声源，不能仅凭这些结果判断它是不是讲话人。采集结束后，系统再由离线L5对L4音频计算逐20 ms人声概率，供人工检查和后处理筛选；它不会在当前采集过程中实时删除风扇方向。由于BF方向分离能力有限，另一方向的人声串入仍可能使L5误判。
...
- 下右L6只在L4关闭“合并”、A/B候选均完成L5后允许手动运行；结果按声纹显示Speaker A/B/C，标出关联音轨数、来源L2 ID和平均MOS。
...
- MUSIC候选阈值、Gate阈值、ID关联门限、卡尔曼Q/R、2秒coasting TTL和3秒静止判定历史需要用真实诊室数据标定；
