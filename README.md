...
IngestCoordinator：建立唯一session / epoch / 绝对sample时间轴
                        ↓
WindowAssembler
    【已完成】每20 ms生成一个DecisionWindow [7680,8]
    每个窗口包含最近160 ms音频和8个对齐的IMCRA结果
                        ↓
WindowWorkItem
    冻结该窗口使用的配置
...
Layer 3：按候选方向增强音频（BF）
    输入：同一160 ms LogicalAudio + 0～3个候选角度
    当前可用：
...
WindowAssembler累计160 ms上下文，之后每20 ms产生一个新窗口。因此算法每秒最多形成50个判断点，但每个判断点都能看到此前完整的160 ms音频。L2、L3、L4使用同一个WindowKey，不能各自重新读取“当前最新音频”。
...
SRP-PHAT比较7个物理麦克风之间的相位时延，只使用2000～4000 Hz频率，在0～359°逐度计算空间响应。响应经过Normalization后寻找圆周局部峰，再执行45°圆周非极大值抑制，最终输出0～3个候选方向。
...
Layer 4判断L3输出的每条方向音频中是否存在人声。它先根据8个20 ms IMCRA概率对模型输入副本做受限响度补偿，再降采样到16 kHz，交给MarbleNet输出Voice / Non-Voice概率。当前CNN采用NVIDIA发布的多语言预训练模型，训练数据包含中文和英文，但英文训练数据来源多于中文；在尚未使用诊室或R6+1目标数据进行微调的情况下，预期英文人声检测表现可能优于中文。
...
3. 点击左上角“启动采集”。首次形成正式窗口前需要累计160 ms音频，IMCRA也会显示预热状态。
