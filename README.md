...
【已完成】Layer 4：采集后1/2人语音分离与主候选匹配
    ...
    └── 2人：MossFormer2或TIGER → 两条匿名候选
          ...
          1～4 kHz复频谱相干初筛 → 按决定时间对齐其他L3方向轨并惩罚串轨
          → 保留原track_id和theta_deg的单条主输出
...
离线L4/L5不在20 ms Runtime中执行。L2按权威ID先登记绝对20 ms时间槽，L3只把BF波形填入对应槽；未生成BF的槽保留等时静音，因此不同L3算法只改变内容而不改变轨长。停止采集并排空L3后，`TrackAudioStreamHub.seal()`直接封存完整ID长音频和逐窗L2方向数量。完整模拟输入EOF等待全部阶段数据完整排空，实时/手动停止才使用有限安全超时。Test UI下半区按L3/L4/L5三等分：由“发送到L4”运行人数路由、分离和1～4 kHz复频谱相干初筛；批处理中再依据各轨`start_sample/end_sample`决定时间，只在至少2秒的真实重叠区间比较初选候选与其他L3方向轨，若初选比同胞候选更像其他ID至少0.025则改选同胞。短于2秒或初筛低可信时仍安全回退原L3人声，并保存带原ID/角度的L4 WAV供试听。整批L4完成后自动运行NVIDIA长音频Frame-VAD。L5为L4音频的每个20 ms hop输出独立概率和Voice判断，黄色逐帧绘制在L4音频条，L3音频不再着色。
