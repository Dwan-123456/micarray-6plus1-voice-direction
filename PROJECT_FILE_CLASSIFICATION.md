...
> 当前开发版本为1.3.1，最终发布基线为`v1.2.4`；职责以[`ARCHITECTURE_V1.1_TARGET.md`](ARCHITECTURE_V1.1_TARGET.md)为准。L1～L5、Rolling NormMUSIC、永久公共方向ID、各UI和正式录音数据系统均纳入版本管理。
...
- `gui/`：Development Test UI消费有序Join快照和容量1的L5完成帧显示邮箱；后者仅降低CNN显示等待，不改变正式结果、录音或watermark顺序。Production UI仍按其当前实现边界处理。
...
- `data/`：当前运行产生的数据、Catalog和scratch资产；`data/external_sources/`另含L5公开bootstrap数据、许可、hash和清单，不属于旧项目源码。
