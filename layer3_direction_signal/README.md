...
L3接收同一160 ms、48 kHz的逻辑8通道音频、L2的0～3个平滑候选角度，以及窗口内8个连续20 ms IMCRA结果。方向增强的目标有效频带为**80～8000 Hz**。内部波束形成只使用前7个物理麦；第8路HardwareMix保留在输入接口中，但不参与导向矢量、协方差矩阵或麦对计算。4个及以上候选属于接口错误，不会截断。L3不接收L2内部ID、不再次滤波角度，也不产生仅供UI的预测方向额外批次。
...
L3保持现有`n_fft=1024、win_length=960、hop_length=480、center=true、reflect`定义及完整160 ms输出。DecisionWindow按绝对sample前进`N=1～7`个20 ms hop且仍有上下文重叠时，新17帧STFT复用`15-2N`个对齐内部帧，只重算当前反射边界和新增的`2+2N`帧；正常`N=1`时复用13帧、重算4帧。IMCRA插值状态只搬运新增N个hop，加权空间协方差分子/分母按相同过期/新增帧集合滚动。BF权重仍按当前窗口和候选角度重新求解，不改变算法选择或公共输出。

所有时间相关缓存使用固定容量状态：正常只保留当前160 ms（8个20 ms hop），绝不超过1000 ms（50 hop）。steering vector与空间`p`结果继续采用原有16项LRU，本次不修改其精确角度key或量化策略。session、epoch、非960 sample对齐、时间倒退、跳跃达到160 ms、STFT配置、IMCRA版本/频率轴、阵列几何或处理设备变化时不得沿用不兼容状态并全量重建。缓存张量驻留处理设备，不向其他层发布。
...
- 音频重建和跨窗口连续性。
- 连续窗口13帧STFT逐元素复用等价、2/4/7 hop跳窗复用与滚动协方差数值等价、达到160 ms无重叠时的失效边界及缓存容量上限。
