...
def _mvdr(
    ...
    """Use the current batched Cholesky MVDR implementation for one retry."""
    steering_fcm = steering_mfc.permute(1, 2, 0).contiguous()
    factor, matrix_ok = _factorize(covariance_fcc, condition_limit)
    solved_fcm = torch.cholesky_solve(steering_fcm, factor)
    weights_rmfc, valid_rmf = _mvdr_from_solved(
        solved_fcm[None], steering_fcm, matrix_ok[None], tolerance,
    )
    return weights_rmfc[0], valid_rmf[0]
