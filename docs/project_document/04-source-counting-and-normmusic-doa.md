...
后续可考虑降低完整声源数判断的执行频率，减少重复空间扫描；也可增加非对称保持策略：稳定检测为2声源后，在短时间内继续保持MUSIC阶数为2，再允许降回1阶。这样能降低1/2阶在边界场景中的快速抖动。保持时间、降阶条件和额外延迟需要用双源出现/消失数据单独验证。

## 11. Gate关闭时仍持续估计

默认声源数估计与P Gate解耦。主要目的是防止Gate从CLOSED变为OPEN时仍没有可用声源数数据：计数器在Gate关闭期间已经维护连续160 ms空间状态并完成稳定投票，Gate开启后可以立即提供最近的0/1/2估计，无需从`None`重新等待两次一致判断。
...
项目历史中评估过神经网络CountNet类方法。该方案计算速度慢，现场效果也不好，未满足项目需求，因此当前`v1.4.3`已删除模型、依赖和运行入口。
...
项目还测试过在MUSIC前加入DPD rank-1频点筛选和基于IMCRA噪声协方差的whitening。两者会增加逐频计算、矩阵处理和候选筛选成本，压缩20 ms实时预算，并可能触发自适应降频；历史真实麦克风对照中，仅开启Whitening就使L2界面采样耗时由约`4.2～6.1 ms`增加到约`6.7～15.9 ms`。现场效果不如不加DPD和Whitening的固定频段NormMUSIC，因此当前不建议使用这两项处理；`v1.4.3`已经删除DPD、IMCRA空间噪声协方差、Whitening配置和运行入口。
...
这个200 ms门槛用于确认持续活动，并保证MUSIC滚动协方差已经达到19帧。Gate连续OPEN门槛必须始终与MUSIC滚动协方差长度一致；当前`context_ms=200`，因此要求连续OPEN 200 ms。它与单个160 ms `DecisionWindow`是两个不同概念。

`Layer2Pipeline`直接按`scan_config.context_ms // 20`计算所需hop数，不单独硬编码200 ms。

后续把MUSIC协方差长度调整为160、240或320 ms时，Gate连续OPEN门槛也必须对应调整为8、12或16个hop，并重新验证ID出生延迟和误报。
