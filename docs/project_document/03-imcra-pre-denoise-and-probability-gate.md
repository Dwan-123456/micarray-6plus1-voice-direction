...
完整旧系统`v1.3.6`另有L5人声识别链，使用NVIDIA `Frame_VAD_Multilingual_MarbleNet_v2.0` CNN对下游16 kHz音频逐20 ms输出Voice/Non-Voice概率。该CNN执行语义级人声判断，与当前P Gate的宽带声学证据用途不同。`v1.4.3`已经移除L5、NVIDIA模型及其推理依赖。
...
它主要为0声源或宽带语音证据不足的窗口提供前置过滤。NormMUSIC先把各有效频率的空间谱按该频率最大值归一化，融合后再做全角min-max归一化，因此最大方向会被推到归一化峰值；即使输入只有噪声，也仍会产生一个相对最大方向。这个最大峰只表示“360个候选角中相对最高”，不能单独证明存在真实声源。P Gate在运行MUSIC前拦截证据不足的窗口，降低这类必然相对最大峰造成的误定位。它不能保证：
...
当前项目不建议开启输入侧预降噪，并保持默认关闭。更合适的处理位置是在方向检测、波束形成或目标音频提取完成后，对输出音频单独降噪；这样不会改动GCC-PHAT、声源数估计和MUSIC共同使用的原始多麦空间关系。当前`v1.4.3`只实现L1/L2，没有输出音频处理链。
