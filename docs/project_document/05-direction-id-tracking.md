...
ID在同一个`session_id`内单调递增。epoch变化会清空活动轨迹，但不复用该session已经分配过的ID；新session从1开始。用户关闭ID追踪后再开启，会执行显式重置。
...
重新开启从新的ID状态开始。若用户在两个L2窗口之间快速“关→开”，独立reset revision确保下一窗仍执行重置，不会因为最终开关又是ON而漏掉清空。
