...
本章建立在一个固定场景假设上：同时存在的两个讲话人在整个追踪期间始终保持不小于50°的圆周角距。当前方向追踪不处理两人方向重叠、相互交叉或交换方位；进入这些场景时，50°取峰间距可能把两个峰压成一个，方向ID也不承诺保持原人物对应关系。
...
这里的“两个峰分数交换”只表示两个始终相隔至少50°的候选在排序上发生变化，不表示系统支持两条人物方向轨迹交叉或交换方位。
...
测试覆盖跨0°、候选rank交换、tentative确认、两秒coasting、长滑行重获原ID、概率降到底仍保活、JPDA一对一、最多4轨迹、出生抑制、50°救援、预测漂移时用最后观测救援、交替重复合并、同时峰不合并、动态确认窗和复用TTL。这些测试不构成双人方向重叠、交叉或交换场景的支持或验收。
