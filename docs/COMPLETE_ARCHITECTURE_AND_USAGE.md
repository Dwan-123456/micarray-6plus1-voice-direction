...
> 适用版本：项目`1.3.1`，发布基线`v1.3.1`。本文根据当前代码、`config/config.yaml`和各层公开数据类型编写，供第一次接触项目的开发者和测试人员使用。
...
flowchart TB
    ...
    subgraph OFFLINE[采集停止后的离线链]
      ...
      MATCH[候选匹配与回退<br/>2–4 kHz复相干度<br/>排列修复·交叉淡化]
...
| Layer 4 | Hub封存的`Layer4LongAudioInput`：完整48 kHz mono、ID、角度、L2方向数历史 | 48→16 kHz；人数路由；1人旁路；2人MossFormer2/TIGER；30秒分块/1秒重叠；排列修复；交叉淡化；2–4 kHz复相干匹配；低可信回退 | `Layer4ProcessedAudio`及16 kHz mono波形；保留父`track_id/theta` | 最短2秒；离线整轨处理 |
...
9. L4整批完成后L5自动运行一次。L4栏试听16 kHz结果并查看黄色Voice区间；L5栏查看逐轨概率和整轨判断。
10. 调整L5阈值只会重判缓存概率和重绘，不会重跑CNN，也不会影响L2 Gate。
...
- 1.3.1详细架构契约：[`../ARCHITECTURE_V1.1_TARGET.md`](../ARCHITECTURE_V1.1_TARGET.md)
...
发生冲突时，当前代码和严格加载的`config/config.yaml`描述实际运行行为，`ARCHITECTURE_V1.1_TARGET.md`描述1.3.1契约；本文用于把两者组织成可操作的交接说明，不建立第二份配置来源。
