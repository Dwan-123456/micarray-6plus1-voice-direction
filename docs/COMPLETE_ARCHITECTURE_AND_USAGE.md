...
| Layer 1 | Host PCM16 `[N,8]`、校准参数、CDC状态 | PCM16解码；增益/极性/整数延迟校准；Host→Logical通道重排；连续性检查；7麦IMCRA；可选IMCRA-Wiener WOLA | `DecodedAudio`：Logical float32 `[N,8]`；Native `[N,8]`；IMCRA噪声PSD/SPP/20 ms声源概率；健康事件 | IMCRA 0–10 kHz；Gate证据100–1500 Hz，男女LTASS固定加权 |
...
| Gate关闭、无方向 | 20 ms概率未达到门限 | L1 IMCRA状态、100–1500 Hz LTASS加权概率和Gate阈值 |
