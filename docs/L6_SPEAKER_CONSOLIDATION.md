...
L3 按 L2 ID/角度形成的长音频
  ...
  -> 用户手动运行 L6
  -> 合并短静音间隙，再将连续有声区限制为 1.5 s 声纹分析窗
...
L5 的连续 Voice 不等于单个说话人轮次。L6 因此对长有声区按
`minimum_embedding_speech_ms` 分窗，使同一 L2 音频内先后出现的不同声纹可以分类。
...
- `Q_mos`：DNSMOS P.835 的 SIG/BAK/OVRL 组合并归一化。
...
- `ApplicationRuntime.build_offline_l6_pipeline()`：构建人工触发的 CPU L6。
